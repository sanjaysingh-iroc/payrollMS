...
function DHTMLgoodies_createStandardObjects()
{
	...
	clientInfoObj.init();	
	if(!dhtmlSuiteConfigObj){	// If this object isn't all ready created, create it.
...
//		var returnValue = inputObj.clientLeft-inputObj.clientLeft;
	  while((inputObj = inputObj.offsetParent) != null){
	  	if(inputObj.tagName!='HTML'){
...
//			el.style.top = (dhtmlSuiteCommonObj.getTopPos(this.dragDropSourcesArray[no][0]) - this.dragDropSourcesArray[no][0].clientX) +'px';
//			el.style.left = (dhtmlSuiteCommonObj.getLeftPos(this.dragDropSourcesArray[no][0]) - this.dragDropSourcesArray[no][0].clientY) +'px';
	...
	__initDragDropElement : function(e)
	{
		...
		referenceToDragDropObject.mouse_x = e.clientX+750;
		referenceToDragDropObject.mouse_y = e.clientY+240;
...
//		referenceToDragDropObject.mouse_y = e.clientY;
	...
	__moveDragableElement : function(e)
	{
		...
		if(referenceToDragDropObject.currentEl_allowX){			
			...
			//====start parvez====
//			dragObj.style.left = (leftPos-750) + 'px'; 
			//====end parvez=====
...
//			dragObj.style.top = (topPos-240) + 'px'; 
			//end parvez====
		}
	}
	// }}}	
	...
	__stop_dragDropElement : function(e)
	{
		...
		for(var no=0;no<referenceToDragDropObject.dragDropTargetArray.length;no++){
			...
			if(leftPosMouse > leftPosEl && leftPosMouse < (leftPosEl + widthEl) && topPosMouse > topPosEl && topPosMouse < (topPosEl + heightEl)){
				if(referenceToDragDropObject.dragDropTargetArray[no][1])eval(referenceToDragDropObject.dragDropTargetArray[no][1] + '("' + referenceToDragDropObject.dragDropSourcesArray[referenceToDragDropObject.numericIdToBeDragged][0].id + '","' + referenceToDragDropObject.dragDropTargetArray[no][0].id + '",' +parseInt( e.clientX) + parseInt(Math.max(document.body.scrollLeft,document.documentElement.scrollLeft))  
						+ ',' + parseInt(e.clientY) + Math.max(document.body.scrollTop,document.documentElement.scrollTop)+')');
...
//		var targetY = dhtmlSuiteCommonObj.getTopPos(referenceToDragDropObject.dragDropSourcesArray[numId][0]);
		//====start parvez====
		var targetX = dhtmlSuiteCommonObj.getLeftPos(referenceToDragDropObject.dragDropSourcesArray[numId][0])-750;
		var targetY = dhtmlSuiteCommonObj.getTopPos(referenceToDragDropObject.dragDropSourcesArray[numId][0])-240;
		//====end parvez====
