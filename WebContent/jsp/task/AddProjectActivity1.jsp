...
<% } %>
 ...
 <%
	...
	//System.out.println("strTaskList ===>> " + strTaskList);
	String strTaskBillableFlag = (String) request.getAttribute("taskBillableFlag");
	boolean taskBillableFlag = strTaskBillableFlag!=null && uF.parseToBoolean(strTaskBillableFlag) ? true : false;
	...
	function addTask(divElement) {
	    ...
	    divTag.innerHTML = 
			...
			"<div style=\"float:left;width:225px;\">"+
				"<select name=\"strTask\" id=\"strTask_"+checkboxcnt+"\" class=\"validateRequired\" onchange=\"checkIsBillable(this.value, '"+checkboxcnt+"','"+cnt+"');\">"+
				"<%=strTaskList%>"+
				"</select>"+
			"</div>"+
			...
			"<div style=\"float: left; width: 55px; text-align: center;\"><input type=\"hidden\" style=\"width: 30px;\"  id=\"strBillableYesNoT_"+checkboxcnt+"\" name=\"strBillableYesNoT\" <%=taskBillableFlag ? "value='1'": "value='0'"%> >"+
			"<span id=\"isBillableSapn"+checkboxcnt+"\"><input type=\"checkbox\" id=\"strBillableYesNo_"+checkboxcnt+"\" name=\"strBillableYesNo\" onchange=\"setBillableValue('"+cnt+"','"+checkboxcnt+"')\" <%=taskBillableFlag?"value='0' checked":"value='0' disabled=disabled" %> ></span></div>"+
...
function checkHours(count) {
	...
	function sortByEmpTimesheets(val) { 
		...
		if(parseInt(cnt) == 0) {
			...
			$.ajax({
				type : 'POST',
				url: 'AddProjectActivity1.action?submitType=LOAD',
				data: form_data,
	...
	function addTaskActivity() {
		...
		$.ajax({
			type : 'POST',
			url: 'AddProjectActivity1.action?submitType=LOAD&submit1=Add Activity',
			data: form_data,
			success: function(result) {
				$("#divResult").html(result);
	   		}
	...
	function taskDaywiseTimeUpdate(strPaycycle, strEmpId, frmDate, toDate, strProject, strActivity, strActivityTaskId, filterBy, strYear, strMonth, pageType, fillUserType) {
		$("#divResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$.ajax({
			url: 'AddProjectActivity1.action?strPaycycle='+strPaycycle+'&strEmpId='+strEmpId+'&frmDate='+frmDate+'&toDate='+toDate+'&strProject='+strProject
	...
	function saveTimesheet() { 
		var form_data = $("#frmProjectActivity1").serialize();
		//alert("form_data ========>> " + form_data);
		$("#divResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$.ajax({
			type : 'POST',
			url: 'AddProjectActivity1.action?submitType=LOAD&save=Save Timesheet',
			data: form_data,
			success: function(result) {
				$("#divResult").html(result);
	   		}
	...
	function checkIsBillable(taskId, checkboxcnt, cnt) {
		var showMessage = '<%=uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_SHOW_BILLABLE_NONBILLABLE_TASK_MESSAGE)) %>';
		$.ajax({
			...
			success : function(data) {
				...
				if(fromData == 'NO') {
					if(showMessage === 'true'){
						alert("Selected task is Non-billable");
					}
					...
					if(showMessage === 'true'){
						alert("Selected task is Billable");
					}
...
<%-- <jsp:include page="../common/SubHeader.jsp">
        ...
        <section class="content" style="padding-top: 0px !important;" >
          <!-- title row -->
           ...
           <%
			...
			<s:form name="frmTimesheet" id="frmTimesheet" action="AddProjectActivity1" theme="simple">
				<div class="col-md-12">
	...
	<!-- ===start parvez date: 28-03-2023=== -->	
		<% //empApproved!=null
			if(uF.parseToInt((String)request.getAttribute("nApproved"))==0 && strUserType!=null && (strUserType.equalsIgnoreCase(IConstants.EMPLOYEE) || 
				...
				|| strUserType.equalsIgnoreCase(IConstants.CEO) || strUserType.equalsIgnoreCase(IConstants.HOD)  || strUserType.equalsIgnoreCase(IConstants.RECRUITER)
				|| strUserType.equalsIgnoreCase(IConstants.OTHER_HR))) {
		%>
	<!-- ===end parvez date: 28-03-2023=== -->	
			<s:form id="frmProjectActivity1" action="AddProjectActivity1" name="frmProjectActivity1" cssClass="formcss" method="post" theme="simple">		
			...
			if(request.getAttribute("sbTasks") != null && !request.getAttribute("sbTasks").equals("")) { %>
				...
				<div class="box box-body" style="overflow-x:scroll;height: 350px;">	
					<div style="float:left; width:750px;" id="div_tasks">
						<div style="float:left; width:750px; padding:2px; boder-bottom: 1px solid #999999">
					    ...
					    <div style="float:left; width:750px; padding:2px;">
						    ...
						    <% if(hmFeatureStatus==null || (hmFeatureStatus!=null && !uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET)))){ %>
						    	<div style="float:left;width:50px;font-weight:bold;">On-Site</div>
						    <% } %>
		...
		<% if(strUserType != null ) { //&& strUserType.equalsIgnoreCase(IConstants.EMPLOYEE)%>
			...
			Apply for: <s:select theme="simple" name="strProType" id="strProType" headerKey="" headerValue="Select Action" 
				...
				<!-- '1':'Reimbursement', '2':'Leave Encashment',  <a href="javascript:void(0)" onclick="addLeave('false')"><input type="button" class="btn btn-primary" style="margin-right: 5px; padding: 3px;" value="Apply Leave"></a> 
				<a href="javascript:void(0)" onclick="addLeave('true')"><input type="button" class="btn btn-primary" style="padding: 3px;" value="Apply Client Weekly Off"></a> -->
			</div>
		<% } %>
	...
	<s:form name="timesheet" id="ftimesheet" action="AddProjectActivity1" theme="simple">
		<div class="col-md-12">
			...
			<div class="box box-body">
				<div class="box-header with-border">  <!-- style="padding: 5px;" -->
					...
					<table class="table table-hover">
						<thead>
							...
							<% } %>	
								...
								/* System.out.println("uF.getDateFormat((String)alDates.get(i), IConstants.DATE_FORMAT, dd) ===>>" + uF.getDateFormat((String)alDates.get(i), IConstants.DATE_FORMAT, "dd"));
								if(uF.getDateFormat((String)alDates.get(i), IConstants.DATE_FORMAT, "dd").equals("01") || uF.getDateFormat((String)alDates.get(i), IConstants.DATE_FORMAT, "dd").equals("02")) {
									System.out.println("strBgColor ===>> " + strBgColor);
								} */
								if(strBgColor==null) {
								...
								//if(uF.getDateFormat((String)alDates.get(i), IConstants.DATE_FORMAT, "dd").equals("01") || uF.getDateFormat((String)alDates.get(i), IConstants.DATE_FORMAT, "dd").equals("02")) {
									//System.out.println("alEmpCheckRosterWeektype ===>> " + alEmpCheckRosterWeektype + " -- strBgColor ==>> " + strBgColor);
									//System.out.println("alDates.get(i) ===>> " + alDates.get(i) + " -- strText ==>> " + strText + " -- weeklyOffSet ===>> " + weeklyOffSet);
								//}
								String bgcolor=null;
								if(strBgColor==null) {
					...
					for(int a=0; alTaskIds != null&& alTaskIds.size()>0 && a<alTaskIds.size(); a++) {
						...
						if((hmTasks == null || hmTasks.isEmpty()) && (alSubTaskIds ==null || alSubTaskIds.size()==0)) {
							...
							//===start parvez date: 27-12-2022===	
								int nCnt = uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET)) ? 1 : 2;
								System.out.println("nCnt=="+nCnt);
								//for(int j=0; j<2; j++) {
								for(int j=0; j<nCnt; j++) {	
							//===end parvez date: 27-12-2022===		
									sb.replace(0, sb.length(), "");
									if(!uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET))){
										if(j==0) {
											sb.append("OFS"); 
										} else {
											sb.append("ONS");
								...
								<% if(uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET))){ %>
									<% if(j==0 && nCount==1) {
											...
											int billrwspn = 0;
											if(nProBillCount>0) {
												billrwspn = nProBillCount;
											}
											int billrwspnT = 0;
											if(nProBillCountT>0) {
												billrwspnT = nProBillCountT;
											}
									%>
							...
							<% } %>
								...
								<% if(hmTaskIsBillableT != null && uF.parseToBoolean(hmTaskIsBillableT.get(strActivityId))) { %>
									...
									<%
									int nCnt1 = uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET)) ? 1 : 2;
						...
						if( hmCheckEmp != null && hmCheckEmp.size()>0 && strActivityId != null && !strActivityId.equals("")) {
							checkEmpList = hmCheckEmp.get(strActivityId.trim());
						}
						...
						if(!checkEmpList.contains(strSessionEmpId) && strUserType.equalsIgnoreCase(IConstants.HRMANAGER) && (pageType == null || !pageType.equals("MP")) && (fillUserType == null || !fillUserType.equals("MY"))) {
							flag = true;
						}
						...
						//System.out.println("hmTaskDescri --->> " + hmTaskDescri);
						...
						<%if(j==0 && nCount==1) { 
						int rwspn = uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET)) ? 1 : 2;
						...
						<% } %>
							...
							<% String strText = "-";
								...
								if(i==0 && j==0) {
									int tskRwSpan = uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET)) ? 1 : 2;
							...
							//	}
								...
								<td align="right" style="background-color: <%=bgcolor==null ? strBgColor:bgcolor %>">
									...
									//System.out.println(alDates.get(i)+" -- hmCheckUserApproval ===>> " + hmCheckUserApproval + " --  ===>> " + hmCheckUserApproval != null ? hmCheckUserApproval.get(taskId+"_"+strSessionEmpId): "---");
									//System.out.println(alDates.get(i)+" -- hmCheckUserApproval ===>> " + hmCheckUserApproval +" -- fillUserType ===>> " + fillUserType+" -- dayStatus ===>> " + dayStatus);
									//System.out.println("strSessionEmpId ===>> " + strSessionEmpId+"-- strEmpId ===>> " + strEmpId);
									//System.out.println("pageType ===>> " + pageType + " -- flag ===>> " + flag);
									if(hmCheckUserApproval!=null && uF.parseToBoolean(hmCheckUserApproval.get(taskId+"_"+strSessionEmpId))==true) {  %>
				
									<% if(hmTaskDescri != null && hmTaskDescri.get(taskId+"_"+(String)alDates.get(i)+"_"+sb.toString()) != null && !hmTaskDescri.get(taskId+"_"+(String)alDates.get(i)+"_"+sb.toString()).equals("") && uF.parseToDouble(dayStatus) > 0) { %>
										<a href="javascript:void(0);" onclick="showTaskDescription('<%=URLEncoder.encode(hmTaskDescri.get(taskId+"_"+(String)alDates.get(i)+"_"+sb.toString()), "UTF-8") %>');"> <span class="worklate" style="height: 10px;">&nbsp;</span></a>
									<% } %>
									<%=dayStatus %>
									
									<% } else if(flag && strUserType.equalsIgnoreCase(IConstants.HRMANAGER) && (pageType == null || !pageType.equals("MP")) && (fillUserType == null || !fillUserType.equals("MY"))) { %>
									...
									<% } else {
										//System.out.println("isApproved else ===>> " + isApproved);
							...
							if(activityType == null || (activityType != null && activityType.equals("E") && (currDt.before(frmDt) || currDt.after(toDt)) || !strActivity.equals(strActivityId))) {
								...
								if(uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_DISABLE_ON_SITE_IN_TIMESHEET))){
									dblDayHrs = uF.parseToDouble((String)hmDates.get((String)alDates.get(i)));
								}
					...
					for(int st=0; alSubTaskIds != null && st<alSubTaskIds.size(); st++) {
						...
						// && hmEmpByLocation.get(request.getAttribute("strEmpId"))==null
						if(!checkEmpListST.contains(strSessionEmpId) && strUserType.equalsIgnoreCase(IConstants.HRMANAGER) && (pageType == null || !pageType.equals("MP")) && (fillUserType == null || !fillUserType.equals("MY"))) { 
							flagST=true;
						}
						...
						<tr id="tr4">
							...
							<% } %>	
								...
								<td align="right" style="background-color: <%=bgcolor==null ? strBgColor : bgcolor%>">
									...
									<% } else if(flagST && strUserType.equalsIgnoreCase(IConstants.HRMANAGER) && (pageType == null || !pageType.equals("MP")) && (fillUserType == null || !fillUserType.equals("MY"))) { %>
		...
		function unLockStatus() {
			...
			if(selectID=="") {
				...
				if(confirm(' Are you sure you want to unlock this timesheet for the employee to modify it?')){
					...
					$.ajax({
						type : 'POST',
						url: 'AddProjectActivity1.action?submitType=LOAD&unlock=Unlock Timesheet',
						data: form_data,
						success: function(result) {
							$("#divResult").html(result);
				   		}
