...
function submitForm(type){
	document.frmMonthlyReceiptReport.exportType.value='';
	var data = "";
	if(type == '1') {
		var f_org = document.getElementById("f_org").value;
		data = '&f_org='+f_org;
	} else if(type == '2') {
		data = $("#frmMonthlyReceiptReport").serialize();
	}
	$("#actionResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
	$.ajax({
		type : 'POST',
		url: 'MonthlyReceiptReport.action',
		data: data,
...
<div class="box-body" style="padding: 5px; overflow-y: auto; min-height: 600px;">
		<div class="desgn" style="margin-bottom: 5px;color:#232323;">
            <div class="box box-primary collapsed-box" style="border-top-color: #EEEEEE; margin-top: 10px;">
                ...
                <div class="box-body" style="padding: 5px; overflow-y: auto; display:none;">
                    <s:form name="frmMonthlyReceiptReport" id="frmMonthlyReceiptReport" action="MonthlyReceiptReport" theme="simple">
		...
		<display:table name="reportList" cellspacing="1" class="table table-bordered" id="lt1">
			<%
				for (int i =0; partiNameList != null && i<partiNameList.size(); i++) {
					String strTitle = partiNameList.get(i);
					if(i<=4) {
			%>
				<display:column nowrap="nowrap" title="<%=strTitle %>" > <%=((java.util.List) pageContext.getAttribute("lt1")).get(i)%></display:column>
			<% } else { %>
				<display:column nowrap="nowrap" styleClass="alignRight padRight20" title="<%=strTitle %>" > <%=((java.util.List) pageContext.getAttribute("lt1")).get(i)%></display:column>
			<% } %>
		<% } %>
		</display:table>
