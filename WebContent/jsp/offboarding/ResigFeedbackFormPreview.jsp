...
function getStepTabContent(EmpId,resignId,formId,currentLevel,levelCount){
    ...
    $.ajax({
    	url: "ResigFeedbackFormPreview.action?strEmpId="+EmpId+"&resignId="+resignId+"&formId="+formId+"&currentLevel="+currentLevel+"&levelCount="+levelCount, 
    	type: "GET",
...
$(function(){
	$("input[type='submit']").click(function(e){
		...
		$("#queAsnDiv").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$.ajax({
	    	url: "ResigFeedbackFormPreview.action", 
	    	type: "POST",
	    	data: form_data,
	    	success: function(result){
	        	$("#queAsnDiv").html(result);
...
<div class="leftbox reportWidth">
		<div id="queAsnDiv">
			...
			<div class="step-tab-content">
				...
				<form action="ResigFeedbackFormPreview.action" id="ResigFeedbackFormID" method="POST" <%if (alSectionId.size() == size) {%>target="_parent"<%} %>>
 ...
 %>
					<div>
						a) <input type="checkbox" value="a"
							<%if (innerMp.get("ANSWER") != null && innerMp.get("ANSWER").contains("a")) {%>
							checked <%}%> name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" disabled/><%=questioninnerList.get(2)%><br />
						...
						c) <input type="checkbox" value="c"
							<%if (innerMp.get("ANSWER") != null && innerMp.get("ANSWER").contains("c")) {%>
							checked <%}%> name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" disabled/><%=questioninnerList.get(4)%><br />
					...
					<br/><b>Comment:</b><br/>
					<textarea rows="3" cols="50" name="anscomment<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" readonly="readonly"  style="width:70%"><%=uF.showData(innerMp.get("ANSWERCOMMENT"),"") %></textarea>
					</div> --%>
	 <%
