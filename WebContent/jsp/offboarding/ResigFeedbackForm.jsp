...
function getStepTabContent(EmpId,resignId,formId,currentLevel,levelCount){
    ...
    $.ajax({
    	url: "ResigFeedbackForm.action?strEmpId="+EmpId+"&resignId="+resignId+"&formId="+formId+"&currentLevel="+currentLevel+"&levelCount="+levelCount, 
    	type: "GET",
...
$("form").bind('submit',function(event) {
       ...
       var resignId = document.getElementById("resignId").value;
		$("#queAsnDiv").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$.ajax({
	    	url: "ResigFeedbackForm.action", 
	    	type: "POST",
	    	data: form_data,
	    	success: function(result){
	        	 if(submit != null && submit == "btnfinish") {
	        		window.location = "ExitForm.action?id="+strEmpId+"&resignId="+resignId;
	        	}else {
	        		$("#queAsnDiv").html(result);
	        	} 
	    	}
...
<div class="leftbox reportWidth">
    <div id="queAsnDiv">
        ...
        <div class="step-tab-content">
            ...
            <%} %>
            <form action="ResigFeedbackForm.action" id="ResigFeedbackFormID" method="POST" <%if (alSectionId.size() == size) {%>target="_parent"<%} %>>
