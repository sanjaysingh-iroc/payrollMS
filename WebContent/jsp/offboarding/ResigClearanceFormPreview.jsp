...
function getStepTabContent(EmpId,resignId,formId,currentLevel,levelCount){
    ...
    $.ajax({
    	url: "ResigClearanceFormPreview.action?strEmpId="+EmpId+"&resignId="+resignId+"&formId="+formId+"&currentLevel="+currentLevel+"&levelCount="+levelCount, 
    	type: "GET",
...
$(function(){
	$("input[type='submit']").click(function(e){
		e.preventDefault();
		var form_data = $("#ResigFeedbackFormID").serialize();
		$("#queAsnDiv").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$.ajax({
	    	url: "ResigClearanceFormPreview.action", 
	    	type: "POST",
	    	data: form_data,
	    	success: function(result){
	        	$("#queAsnDiv").html(result);
		...
		<div id="queAsnDiv">
			...
			<div class="step-tab-content">
				<% if(!request.getAttribute("levelCount").toString().equals("1")) { %>
					...
					<% if(existLevelFlag) {
						%>
							<br/><h1>You Already Filled This Section.</h1><br/>
						<%	
						} else {
						%>
				...
				<form action="ResigClearanceFormPreview.action" id="ResigFeedbackFormID" method="POST" <%if (alSectionId.size() == size) {%>target="_parent"<%} %>>
								...
								<li>
									...
									<div>
										a) <input type="checkbox" value="a" name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>"
											...
											checked <%}%>  disabled/><%=questioninnerList.get(2)%><br /> b) <input
											...
											checked <%}%> value="b" disabled/><%=questioninnerList.get(3)%><br />
										...
										c) <input type="checkbox" value="c"
											...
											checked <%}%> disabled/><%=questioninnerList.get(4)%><br /> d) <input
											type="checkbox" name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>"
											<%if (innerMp.get("ANSWER") != null && innerMp.get("ANSWER").contains("d")) {%>
											checked <%}%> value="d" disabled/><%=questioninnerList.get(5)%><br />

										<textarea rows="5" cols="50" name="<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:100%" readonly="readonly" ><%=innerMp.get("REMARK") != null ? innerMp.get("REMARK") : ""%></textarea>
									</div> 
									<%-- <div id="ansType1cmnt<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>">
									<br/><b>Comment:</b><br/>
									<textarea rows="3" cols="50" name="anscomment<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:70%" readonly="readonly"><%=uF.showData(innerMp.get("ANSWERCOMMENT"),"") %></textarea>
									</div> --%>
									<%
 ...
 %>
					<div>
						a) <input type="checkbox" value="a"
							<%if (innerMp.get("ANSWER") != null && innerMp.get("ANSWER").contains("a")) {%>
							checked <%}%> disabled name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" /><%=questioninnerList.get(2)%><br />
						...
						c) <input type="checkbox" value="c"
							<%if (innerMp.get("ANSWER") != null && innerMp.get("ANSWER").contains("c")) {%>
							checked <%}%> disabled name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" /><%=questioninnerList.get(4)%><br />

						d) <input type="checkbox" name="correct<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>"
							<%if (innerMp.get("ANSWER") != null && innerMp.get("ANSWER").contains("d")) {%>
							checked <%}%> value="d" disabled/><%=questioninnerList.get(5)%>
					</div>
 ...
 %>
				<div>
					...
					<input type="radio" name="<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>"
						...
						checked <%}%> value="<%=inner.get(0)%>" disabled/><%=inner.get(1)%><br />
					<%
						}
					%>
				</div> 
...
<%
 ...
 %>
					<div>
						<textarea rows="5" cols="50" name="<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" readonly="readonly" style="width:100%" readonly="readonly"><%=innerMp.get("ANSWER") != null ? innerMp.get("ANSWER") : ""%></textarea>
...
<%
 ...
 %>
					<div>
						...
						<div>
							<textarea rows="5" cols="50" name="a<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:100%" readonly="readonly"><%=a != null ? a[0] : ""%></textarea>
							<br />
						</div>
						<div style="float: left; margin: 30px 10px 0px 0px;">b)
						</div>
						<div>
							<textarea rows="5" cols="50" name="b<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:100%" readonly="readonly"><%=a != null ? a[1] : ""%></textarea>
							<br />
						</div>
						<div style="float: left; margin: 30px 10px 0px 0px;">c)
						</div>
						<div>
							<textarea rows="5" cols="50" name="c<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:100%" readonly="readonly"><%=a != null ? a[2] : ""%></textarea>
							<br />
						</div>
						<div style="float: left; margin: 30px 10px 0px 0px;">d)
						</div>
						<div>
							<textarea rows="5" cols="50" name="d<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:100%" readonly="readonly"><%=a != null ? a[3] : ""%></textarea>
							<br />
						</div>
						<%-- <input type="text" name="marks<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>"
 ...
 %>
				...
				name="gradewithrating<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" /> 
	...
	<%
	 ...
	 %>
						<div>
							<textarea rows="5" cols="50" name="<%=hmSecQuestion.get("SECTION_QUEST_QUESTION_BANK_ID")%>_<%=questioninnerList.get(9)%>" style="width:100%" readonly="readonly"><%=innerMp.get("ANSWER") != null ? innerMp.get("ANSWER") : ""%></textarea>
						</div> 
				...
				<% } %>
					...
					<div class="clr margintop20">
						...
						<% } else {
								if(request.getAttribute("levelCount").toString().equals("1")) {
						%>
									<s:submit value="View Assessment" cssClass="btn btn-primary" name="submit"></s:submit>
						<% 		} else { %>
