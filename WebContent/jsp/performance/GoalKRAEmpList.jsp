...
%>
	 			...
	 			<li class="item">
	 				<span style="float: left; width: 100%;">
	 					<%if(uF.parseToInt(empId) == uF.parseToInt(strEmpId)) { %>
			 				...
			 				<a href="javascript:void(0);" class="activelink" onclick ="getGoalKRADetails('GoalKRATarget','<%=empId %>','<%=dataType %>','<%=currUserType %>','GKD')" >
			 				<%=hmEmpNames.get(empId) %></a>
			 				 <span style="float: right;margin-right: 5px;" class="label label-primary"><%=kraSize %></span>
			 			<% } else { %>
			 				<img height="20" width="20" class="lazy img-circle" src="userImages/avatar_photo.png"
	                         data-original="<%=docRetriveLocation +IConstants.I_PEOPLE+"/"+IConstants.I_IMAGE+"/"+empId.trim()+"/"+IConstants.I_22x22+"/"+strImage%>" />
			 				<a href="javascript:void(0);"  onclick ="getGoalKRADetails('GoalKRATarget','<%=empId %>','<%=dataType %>','<%=currUserType %>','GKD')" >
			 				<%=hmEmpNames.get(empId)%></a>
			 				 <span  style="float: right;margin-right: 5px;" class="label label-primary"><%=kraSize %></span>
			 			<% } %>
			   		</span>
  ...
  <script type="text/javascript" charset="utf-8">
	$(document).ready(function() {
		getGoalKRADetails('GoalKRATarget','<%=strEmpId%>','<%=dataType%>','<%=currUserType%>','GKT');
...
function loadMore(proPage, minLimit,dataType,currUserType) {
		...
		$.ajax({ 
			type : 'POST',
			url: 'GoalKRAEmpList.action?f_org='+org+paramValues+'&dataType='+dataType+'&currUserType='+currUserType+"&proPage="+proPage+"&minLimit="+minLimit,
			cache: true,
			success: function(result){
