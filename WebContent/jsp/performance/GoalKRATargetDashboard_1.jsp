...
<script>
	...
	function myKRA(currUserType) { 
		...
		$('.modal-title').html('New Business Goal');
		$("#modalInfo").show();
		...
		$.ajax({
			url : 'MyGoalPopUp_1.action?operation=A&type=type&typeas=KRA&fromPage=GKT&strID='+strID+'&currUserType='+currUserType,
			cache : false,
	...
	function myBsc(currUserType) {
		...
		$(dialogEdit).empty();
		$(dialogEdit).html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$('.modal-title').html('New Balance Scorecard');
		$("#modalInfo").show();
 ...
 <section class="content">
    <div class="row jscroll">
        ...
        <div class="box box-primary">
          <div class="box-body">
<% } %>
			<div class="col-md-12"  style=" min-height: 600px;"> <!-- padding-left:0px; -->
				 <% if(isFlag) { %>
					 	<div class="box box-none nav-tabs-custom">
	            	 		<ul class="nav nav-tabs">
	            	 			<li class="active"><a href="javascript:void(0)" onclick="getGoalKRATargetDashboardData('GoalKRATargetDashboardData_1','<%=dataType %>','MYTEAM');" data-toggle="tab">MyTeam</a></li>
	      						<li><a href="javascript:void(0)" onclick="getGoalKRATargetDashboardData('GoalKRATargetDashboardData_1','<%=dataType %>','<%=strBaseUserType %>');" data-toggle="tab"><%=strBaseUserType %></a></li>
	            	 		</ul>
				 ...
				 <%} else { %>
     			 ...
     			 <!-- <div class="nodata msg">This section is not part of this instance. To enable the feature, please write to <a href="mailto:support@workrig.com">support@workrig.com</a> or raise a ticket.
     			 <br/>Also to be noted, that this is an optional feature, therefore an alternate feature may be disabled while enabling this feature.</div> -->
...
<%}%>
 ...
 <script type="text/javascript" charset="utf-8">
	$(document).ready(function() {
		var flag = '<%=isFlag%>';
		if(flag === true) {
			getGoalKRATargetDashboardData('GoalKRATargetDashboardData_1','L','MYTEAM', '');
		} else {
			getGoalKRATargetDashboardData('GoalKRATargetDashboardData_1','L','<%=currUserType%>', '');
		}
	});
	
	function getGoalKRATargetDashboardData(strAction,dataType,currUserType, callFrom){
		...
		$.ajax({ 
			type : 'POST',
			url: strAction+'.action?dataType='+dataType+'&currUserType='+currUserType+'&callFrom='+callFrom,
			cache: true,
