...
<%  UtilityFunctions uF = new UtilityFunctions();
	...
	if(request.getAttribute("download")!=null){
		String downloadPDF=request.getAttribute("download").toString();
	}
%>
...
<script type="text/javascript">
    ...
    function myGoal() { 
    	...
    	var dialogEdit = '.modal-body';
    	 ...
    	 $.ajax({
   			url : "MyGoalPopUp.action?operation=A&type=type&typeas=goal&strID="+strID+"&fromPage=KT&goalTypePG="+<%=IConstants.PERSONAL_GOAL%>,/*Created By Dattatray Date :09-09-21 Note: Added goalTypePG  */
   			cache : false,
   			success : function(data) {
   				$(dialogEdit).html(data);
   			}
    ...
    function editGoal(goalid) { 
    	var dialogEdit = '.modal-body';
    	 ...
    	 $.ajax({
   			url : "EditMyPersonalGoalPopUp.action?operation=E&type=type&goalid="+goalid+"&typeas=goal&from=KT&goalTypePG="+<%=IConstants.PERSONAL_GOAL%>,
   			cache : false,
   			success : function(data) {
				$(dialogEdit).html(data);
   			}
    ...
    function openAppraisalPreview(id, appFreqId) {
    	 ...
    	 $(dialogEdit).empty();
    		 ...
    		 $.ajax({
    		url : "AppraisalPreview.action?id="+id+"&appFreqId="+appFreqId+"&fromPage=KRATARGET",
    		cache : false,
    		success : function(data) {
    			$(dialogEdit).html(data);
    		}
    ...
    function getGoalApprovalStatus(goal_id,empname,type){
		var dialogEdit = '.modal-body';
		$(dialogEdit).empty();
		$(dialogEdit).html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$('.modal-title').html('Work flow of '+empname);
		$("#modalInfo").show();
		$.ajax({
			url : "GetLeaveApprovalStatus.action?effectiveid="+goal_id+"&type="+type,
			cache : false,
			success : function(data) {
				$(dialogEdit).html(data);
			}
...
/* ===start parvez date: 06-08-2022=== */      
	function updateOneOneRatingAndComment(empId, appid, appFreqId, dicussionId, operation) {
	    ...
	    if(strComment==""){
	    	alert("Please Enter Reason");
	         ...
	         if (xmlhttp == null) {
	            ...
	            var xhr = $.ajax({
		            url : 'KRATarget.action?appId='+appid+'&appFreqId='+appFreqId+'&strComment='+encodeURIComponent(strComment)
		            		+'&operation='+operation+'&discussionId='+dicussionId,
		            cache : false,
		            success : function(data) {
		            	getMyHRData('KRATarget','L','','');
		          	},
    	...
    	if(strComment==""){
    		...
    		xmlhttp = GetXmlHttpObject();
	         if (xmlhttp == null) {
	            ...
	            var xhr = $.ajax({
		            url : 'KRATarget.action?appId='+appid+'&appFreqId='+appFreqId+'&operation='+operation+'&discussionId='+dicussionId
		            			+'&strComment='+encodeURIComponent(strComment)+'&apStatus='+apStatus,
		            cache : false,
		            success : function(data) {
		            	getMyHRData('KRATarget','L','','');
		          	},
...
/* ===start parvez date: 30-03-2023=== */
	function getMemberData(memberId, empId, id, role, appFreqId) {
    	var dialogEdit = '.modal-body';
    	$(dialogEdit).empty();
    	$(dialogEdit).html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
    	$("#modalInfo").show();
...
<div class="row jscroll">
	...
	<div class="box box-none nav-tabs-custom">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="javascript:void(0)" style="padding: 5px 10px;" onclick="getKRATargetData('KRATarget','L','<%=fromPage%>')" data-toggle="tab">Live</a>
			</li>
			<li><a href="javascript:void(0)" style="padding: 5px 10px;" onclick="getKRATargetData('KRATarget','C','<%=fromPage%>')" data-toggle="tab">Closed</a>
			</li>
			<% } else if(dataType != null && dataType.equals("C")) { %>
			<li><a href="javascript:void(0)" style="padding: 5px 10px;" onclick="getKRATargetData('KRATarget','L','<%=fromPage%>')" data-toggle="tab">Live</a>
			</li>
			<li class="active"><a href="javascript:void(0)" style="padding: 5px 10px;" onclick="getKRATargetData('KRATarget','C','<%=fromPage%>')" data-toggle="tab">Closed</a>
			</li>
			<% } %>
		</ul>
		<!-- created by seema -->
		<div id="downloadPDF" class="box-body" style="padding: 0px; overflow-y: auto; min-height: 600px;">
		<!-- created by seema -->
			<%=uF.showData((String)session.getAttribute(IConstants.MESSAGE),"") %>
			<section class="content">
			<div class="row jscroll">
			
				<section class="col-lg-4 connectedSortable" style="max-height: 600px !important; overflow-y: hidden;" id="sectionLeft" >
				
	 				<div class="notranslate box box-widget widget-user widget-user1">
	              ...
	              <div class="box box-info">
			          <%if(arrEnabledModules!=null && ArrayUtils.contains(arrEnabledModules, IConstants.MODULE_PERFORMANCE_MANAGEMENT+"")>=0){ 
						...
						Map<String, String> hmEmpAggregateMap =(Map<String, String>) request.getAttribute("hmEmpAggregateMap");
		 				...
		 				for (int i = 0; elementouterList != null && i < elementouterList.size(); i++) {
		 					...
		 					if(attributeouterList1 != null && !attributeouterList1.isEmpty()) {
			 					for (int j = 0; j < attributeouterList1.size(); j++) {
			 						...
			 						double dblAtrribRating = uF.parseToDouble(hmEmpAggregateMap.get(attributeList1.get(0).trim()));
			 						if(dblAtrribRating>0) {
				 						totAllAttribRating += dblAtrribRating;
				 						count++;
			 						}
			 					}
		 					} else {
								...
								%>
								    ...
								    %>
										...
										<div id="starPrimary<%=i %><%=j %>" style="float:left; width: 100px;"></div>
							   			<input type="hidden" id="gradewithrating<%=i %><%=j%>" value="<%=hmEmpAggregateMap.get(attributeList1.get(0).trim()) != null ? uF.parseToDouble(hmEmpAggregateMap.get(attributeList1.get(0).trim())) + "" : "0"%>"; name="gradewithrating<%=i %><%=j%>" />
											<script type="text/javascript">
									        	$('#starPrimary<%=i %><%=j %>').raty({
									        		readOnly: true,
									        		start: <%=hmEmpAggregateMap.get(attributeList1.get(0).trim()) != null ? uF.parseToDouble(hmEmpAggregateMap.get(attributeList1.get(0).trim())) + "" : "0"%>,
									        		half: true,
									        		targetType: 'number',
									        		click: function(score, evt) {
				...
				<section class="col-lg-4 connectedSortable" style="max-height: 600px !important; overflow-y: hidden;" id="sectionFinalReview" >
				<div class="box box-primary" style="border-top-color: #cda55f /*#E0E0E0;*/">
					...
					<div class="box-body" style="padding: 5px; overflow-y: auto; max-height: 250px;">
						<div class="attendance">
							...
							<table width="100%" cellspacing="0" cellpadding="2" align="left" class="table" style="margin-bottom: 0px;">
								<tbody>
									<tr class="darktable">
										<th style="width: 60%; text-align: center; border-top-color: #FFF;">Reviews Name</th>
							...
							<a href="javascript:void(0)" onclick="giveRemark()" data-toggle="tab">Annual Appraisal Report</a>
					...
					<div class="box-body" style="padding: 5px; overflow-y: auto;">
						<div class="details_lables">
							<%
                                          ...
                                          // System.out.println("KRAT.jsp/1176---orientationMemberMp=="+orientationMemberMp);
                                    ...
                                    /* ===start parvez date: 06-07-2022=== */       
                                           Map<String, List<String>> hmUsersFeedbackReopenComment = (Map<String, List<String>>) request.getAttribute("hmUsersFeedbackReopenComment");
                                           if(hmUsersFeedbackReopenComment == null) hmUsersFeedbackReopenComment = new HashMap<String, List<String>>(); 
							...
							<ul style="margin-bottom: 10px; padding-left: 10px;">
								...
								<li class="list">
									...
									<!-- ===start parvez date: 06-07-2022=== -->
											<% 
												if(uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_REVIEW_REOPEN_BY_HR_GHR_FOR_UPDATE_FEEDBACK)) && hmUsersFeedbackReopenComment != null && !hmUsersFeedbackReopenComment.equals("") && !hmUsersFeedbackReopenComment.isEmpty()){
													//System.out.println("hmUsersFeedbackReopenComment="+hmUsersFeedbackReopenComment.get(appId+"_"+appFreqId+"_"+key+"_"+employeeList.get(ii)));
													List<String> feedbackReopenList = hmUsersFeedbackReopenComment.get(appId+"_"+appFreqId+"_"+key+"_"+employeeList.get(ii));
													if(feedbackReopenList != null && feedbackReopenList.size()>0){
											%>
													<br />
													<i>Reopened by: <%=hmEmpName.get(feedbackReopenList.get(1)) %> </i>
									...
									<!-- ===end parvez date: 06-07-2022=== -->
											...
											<span style="float: right; margin-top: 5px; margin-right: 10px;">
												...
												</a> <% } %> <% } %> </span> <% } else { %> <a class="approve-font" title="In progress"></a> <% } %> 
												<a onclick="openAppraisalPreview('<%=appId %>','<%=appFreqId %>')" href="javascript: void(0)" title="Preview"><i class="fa fa-eye" aria-hidden="true"></i></a>
												<a href="MyReviewStatus.action?id=<%=appId %>&appFreqId=<%=appFreqId %>"><img title="Status" src="images1/icons/icons/status_icon.png"></a>&nbsp; 
												<%if(!uF.parseToBoolean(hmAppraisalMp.get("FREQ_IS_CLOSE")) && (!uF.parseToBoolean(hmAppraisalMp.get("APP_FINAL_STATUS")) || !uF.parseToBoolean(hmAppraisalMp.get("APP_STATUS")))) { %>
										...
										<div>
											<%  List<String> innList = hmAppraisalNextStep.get(appId+"_"+appFreqId);
                                                                ...
                                                                if((alSelfFillEmpIds != null && alSelfFillEmpIds.size()>0 && alSelfFillEmpIds.contains(strSessionEmpId) && role != null && role.equals("Self")) || (role != null && !role.equals("Self"))) {
                                                                	...
                                                                	//===start parvez date: 15-03-2023===	
                                                                		if(hmFeatureStatus!=null && uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_GOAL_NOT_SHOWING_IN_PEER_AND_HOD)) && role!=null && 
                                                                				(role.equalsIgnoreCase("Peer") || role.equalsIgnoreCase("Other Peer") || role.equalsIgnoreCase("HOD")) && innerList.get(2)!=null && uF.parseToInt(innerList.get(2))!=2){
                                                                			continue;
                                                                		}
                                                                	//===end parvez date: 15-03-2023===
                                                                		
                                                                		List<String> listRemainOrientName = hmRemainOrientDetails.get(employeeList.get(ii)+"_"+innerList.get(0) + "NAME");
                                                                		List<String> listRemainOrientID = hmRemainOrientDetails.get(employeeList.get(ii)+"_"+innerList.get(0) + "ID");
                                                                		...
                                                                		/* if(uF.parseToInt(hmAppraisalMp.get("ID"))==48) {
                                                                		...
                                                                		for (int b = 0; listRemainOrientID != null && b < listRemainOrientID.size(); b++) {
                                                                			if (listExistOrientTypeInAQA != null) {
                                                                				if (!listRemainOrientID.get(b).trim().equals("") && !listExistOrientTypeInAQA.contains(listRemainOrientID.get(b).trim())) {
                                                                					...
                                                                					/* if(uF.parseToInt(hmAppraisalMp.get("ID"))==33) {
											...
											System.out.println("=== 1"); */
												if (empstatusMp.get(employeeList.get(ii)+"_"+strSessionEmpId) != null && sectionIDList != null && sectionIDList.contains(innerList.get(0)) 
														&& uF.parseToInt(hmSectionQueCnt.get(hmAppraisalMp.get("ID")+"_"+employeeList.get(ii)+"_"+innerList.get(0))) == uF.parseToInt(hmSectionGivenQueCnt.get(key+"_"+hmAppraisalMp.get("ID")+"_"+appFreqId+"_"+"0"+"_"+employeeList.get(ii)+"_"+innerList.get(0)))) {
													/* if(uF.parseToInt(hmAppraisalMp.get("ID"))==33)
                                                ...
                                                %>
												<a href="javascript: void(0);" onclick="staffReviewPoup('<%=hmAppraisalMp.get("ID")%>','<%=employeeList.get(ii)%>','<%=key%>','<%=innerList.get(0)%>','<%=role%>','<%=appFreqId%>')"><%=innerList.get(1) %></a>
											<%--  Section(<%=k + 1%>) staffReviewSummaryPoup( --%>
											<% } else { 
												/* if(uF.parseToInt(hmAppraisalMp.get("ID"))==26)
													System.out.println("=== 3"); */  %>
											<%-- <a href="javascript: void(0);" onclick="staffReviewPoup('<%=hmAppraisalMp.get("ID")%>','<%=employeeList.get(ii)%>','<%=key%>','<%=appFreqId%>')"> --%>
												<a href="javascript: void(0);" onclick="staffReviewPoup('<%=hmAppraisalMp.get("ID")%>','<%=employeeList.get(ii)%>','<%=key%>','<%=innerList.get(0)%>','<%=role%>','<%=appFreqId%>')"><%=innerList.get(1) %></a>
											<% } %> <!-- staffAppraisalPreview( -->
											<% } else if (listRemainOrientType != null && !listRemainOrientType.isEmpty() && !role.equalsIgnoreCase("Self") && !role.equalsIgnoreCase("Peer")) {
												/* if(uF.parseToInt(hmAppraisalMp.get("ID"))==26)
											...
											<br />
											<% secCnt++; 
                                             if(uF.parseToInt(hmSectionQueCnt.get(hmAppraisalMp.get("ID")+"_"+employeeList.get(ii)+"_"+innerList.get(0))) == uF.parseToInt(hmSectionGivenQueCnt.get(key+"_"+hmAppraisalMp.get("ID")+"_"+appFreqId+"_"+"0"+"_"+employeeList.get(ii)+"_"+innerList.get(0)))) {
                                             	sectionFilled = true;
                                             } else {
                                             	sectionFilled = false;
                                             }
										...
										<% if (dblRate >= 3.5) { %>
										<div style="background-color: #00FF00; padding: 0px 3px; margin: 5px; font-family: digital; font-size: 14px; border-radius: 4px 4px 4px 4px; float: left;"><%=("" + dblRate != null) ? uF.formatIntoOneDecimal(dblRate) : ""%>/5
										</div>
										<% } else if (dblRate < 3.5 && dblRate >= 2.5) { %>
										<div style="background-color: #FFFF00; padding: 0px 3px; margin: 5px; font-family: digital; font-size: 14px; border-radius: 4px 4px 4px 4px; float: left;"><%=("" + dblRate != null) ? uF.formatIntoOneDecimal(dblRate) : ""%>/5
										</div>
										<% } else if (dblRate > 0) {  //#FF0000; %>
										<div style="background-color: #FF0000; padding: 0px 3px; margin: 5px; font-family: digital; font-size: 14px; border-radius: 4px 4px 4px 4px; float: left;"><%=("" + dblRate != null) ? uF.formatIntoOneDecimal(dblRate) : ""%>/5
										</div>
										<% } %>
										<% } %>
										<% } %>
				...
				<div class="box box-primary" style="border-top-color: #cda55f /*#E0E0E0;*/">
					...
					<div class="box-body" style="padding: 5px; overflow-y: auto;">
						<div class="details_lables">
								...
								<li class="list">
									<div style="margin-bottom: 10px;">
										...
										<div>
											<%  List<String> innList = hmAppraisalNextStep.get(appId+"_"+appFreqId);
                                             ...
                                             if((alSelfFillEmpIds != null && alSelfFillEmpIds.size()>0 && alSelfFillEmpIds.contains(strSessionEmpId) && role != null && role.equals("Self")) || (role != null && !role.equals("Self"))) {
                                            	...
                                            	//===end parvez date: 15-03-2023===
                                             		...
                                             		List<String> listRemainOrientName = hmRemainOrientDetails.get(reviewedEmpList.get(ii)+"_"+innerList.get(0) + "NAME");
                                             		List<String> listRemainOrientID = hmRemainOrientDetails.get(reviewedEmpList.get(ii)+"_"+innerList.get(0) + "ID");
											...
											<br />
											<% secCnt++; 
                                                  if(uF.parseToInt(hmSectionQueCnt.get(hmAppraisalMp.get("ID")+"_"+reviewedEmpList.get(ii)+"_"+innerList.get(0))) == uF.parseToInt(hmSectionGivenQueCnt.get(key+"_"+hmAppraisalMp.get("ID")+"_"+appFreqId+"_"+"0"+"_"+reviewedEmpList.get(ii)+"_"+innerList.get(0)))) {
                                                  	sectionFilled = true;
                                                  } else {
                                                  	sectionFilled = false;
                                                  }
							...
							<div class="nodata msg" style="width: 96%;">No forms reviewed.</div>
							<% } %>
						</div>
					</div>
					<!-- /.box-body -->
				...
				<div class="box box-primary" style="border-top-color: #cda55f /*#E0E0E0;*/">
					<div class="box-header with-border">
						<h3 class="box-title" style="font-size: 14px !important;">One On One Discussion</h3>
					</div>
					<!-- /.box-header -->
					<div class="box-body" style="padding: 5px; overflow-y: auto;">
						<div class="details_lables">
							...
							//System.out.println("hmAppraisalDetails="+hmAppraisalDetails);
								...
								<%
									...
									while(itr.hasNext()){ 
										...
										if(appraisalList != null && !appraisalList.isEmpty()  && appraisalList.size()>0) {
											List<String> discussionList = hmDiscussionDetails.get(appraisalList.get(0)+"_"+appraisalList.get(11));
											if(discussionList == null) discussionList=new ArrayList<String>();
								%>
											<li>
														...
														<div class="row row_without_margin">
															<div class="col-lg-12 col-md-12 col-sm-12" style="margin-top:1%;">
																<%if(hmFeatureStatus!=null && uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_TEN_STAR_RATING_FOR_REVIEW))){
																	String marks = hmAppraisalFinalScore.get(appraisalList.get(0))!=null ? uF.parseToDouble(hmAppraisalFinalScore.get(appraisalList.get(0))) / 10 +"" : "0" ;
																%>
																	<script type="text/javascript">
									                                  	$(function() {
									                                  		$('#starUserRating<%=appraisalList.get(0)%>_<%=appraisalList.get(11)%>').raty({
											                                    readOnly: true,
											                                    start: <%=marks%>,
											                                    number: 10,
											                                    half: false,
									                                ...
									                                <div id="starUserRating<%=appraisalList.get(0)%>_<%=appraisalList.get(11)%>" style="float: left; margin: 5px 0px; width: 110px;"></div>&nbsp;<%=uF.showData(marks,"0")%>
																<%} else{ %>
																<script type="text/javascript">
								                                  	$(function() {
								                                  		$('#starUserRating<%=appraisalList.get(0)%>_<%=appraisalList.get(11)%>').raty({
								                                ...
								                                <div id="starUserRating<%=appraisalList.get(0)%>_<%=appraisalList.get(11)%>" style="float: left; margin: 5px 0px; width: 110px;"></div>
																<%} %>
														...
														<% if(discussionList != null && !discussionList.isEmpty() && discussionList.size()>0){ %>
															<div class="row row_without_margin">
																...
																<% 
																	...
																	if(discussionList !=null && discussionList.size()>0 && discussionList.get(4)!=null && uF.parseToBoolean(discussionList.get(4))){ 
																		empSignOff = true+"";
																	} 
																	
																%>
																	<div style="float: left; width: 100%;">
																		<span style="float: left; font-size: 12px; line-height: 32px;"><b>Self Comment:&nbsp;&nbsp;		
																	...
																	<div style="float: left; width: 100%;">
																		...
																		<%-- <% if(!uF.parseToBoolean(empSignOff)){ %>
																			<span>
																				<a href="javascript:void(0);" onclick="discussionSignOff('1','<%=strSessionEmpId %>', '<%=appraisalList.get(0) %>', '<%=appraisalList.get(11) %>','<%=discussionList!=null&&discussionList.size()>0? discussionList.get(0):"0" %>', 'SO')">
															...
															<div class="col-lg-12 col-md-12 col-sm-12" style="margin-top:1%;">
																...
																%>
																	...
																	<div style="width: 90%; padding-top: 20px">
																		<textarea rows="1" cols="40" style="width: 90% !important;"
																		 name="strComment<%=appraisalList.get(0)%>_<%=appraisalList.get(11)%>" 
																		 id="strComment<%=appraisalList.get(0)%>_<%=appraisalList.get(11)%>" <%=uF.parseToBoolean(empSignOff)?"readonly":"" %> ></textarea>
																	</div>
																	<%-- <% if(discussionList != null && !discussionList.isEmpty() && discussionList.size()>0){ %> --%>
																		<div style="float: right; margin: 0px 0px 5px 7px; padding-top: 20px; width: 100%;">
																			<% if(uF.parseToBoolean(empSignOff)){ %>
																			...
																			<% } else{ %>
																				<%if(discussionList != null && !discussionList.equals("") && !discussionList.isEmpty() && discussionList.size() > 0 && discussionList.get(2)!=null){ %>
																					<a href="javascript:void(0);" onclick="discussionSignOff('1','<%=strSessionEmpId %>', '<%=appraisalList.get(0) %>', '<%=appraisalList.get(11) %>','<%=discussionList!=null&&discussionList.size()>0? discussionList.get(0):"0" %>', 'SO')">
																						<input type="button" class="btn btn-primary" name="submit" value="Sign Off">
																					</a>&nbsp;&nbsp;
																					<a href="javascript:void(0);" onclick="updateOneOneRatingAndComment('<%=strSessionEmpId %>', '<%=appraisalList.get(0) %>', '<%=appraisalList.get(11) %>','<%=discussionList!=null&&discussionList.size()>0? discussionList.get(0):"0" %>', 'update')">
				...
				<section class="col-lg-4 connectedSortable " style=" max-height: 600px !important; overflow-y: hidden;" id="sectionKra" >
					<div class="box-header with-border">
						<h3 class="box-title" style="font-size: 14px !important;">My Goals</h3>
					</div>
				<!-- <h3 style="margin-top: 0px; font-size: 14px !important; font-weight: 600;" class="pagetitle">My Goals</h3> -->
				...
				<div>
					...
					<a style="height: 1px; width: 2px;" href="javascript:void(0)" onclick="myGoal();" class="" title="Add New Personal Goal">
						<i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;Add New Personal Goal</a>  <!-- Add New Initiative -->
				</div>
				<% } %> <br />
				<%
                    			...
                    			// Created By Dattatray Date:07-09-21
                                    		...
                                    		Iterator<String> itgKRA = hmGoalKra.keySet().iterator();
                                    		 while(itgKRA.hasNext()) {
                                              ...
                                              //Created By Dattatray Date:08-09-21
                                               ...
                                               //	System.out.println("KRATarget.jsp/1868--approveStatus="+approveStatus);
				...
				<div class="box box-primary collapsed-box"
					...
					<div class="box-header with-border" style="height: 40px;">
						<h3 class="box-title" style="font-size: 12px !important;"><%=gInnerList.get(3) %>
					...
					<!-- ===start Parvez date: 08-12-2021=== -->
							<%-- <% if (uF.parseToInt(gInnerList.get(18)) == IConstants.PERSONAL_GOAL && strSessionUserType.equalsIgnoreCase(IConstants.EMPLOYEE)) { %> --%>
							<% if (uF.parseToInt(gInnerList.get(18)) == IConstants.PERSONAL_GOAL && strSessionUserType.equalsIgnoreCase(IConstants.EMPLOYEE)&& uF.parseToInt(gInnerList.get(41)) != 1) { %>
					<!-- ===end Parvez date: 08-12-2021=== -->
							<%if(!uF.parseToBoolean(gInnerList.get(12))) { %>
							<a href="javascript:void(0);"style="float: right;" title="Close Goal" onclick="closeGoal('<%=goalid %>', 'close');"><i class="fa fa-times-circle-o" aria-hidden="true"></i>
							</a>
							<% if(!alCheckList.contains(goalid)) { %>
							<a href="javascript:void(0)" style="float: right;" class="del" title="Delete Goal" onclick="deleteGoal('<%=goalid %>')"><span class="fa fa-trash" style="color: rgb(218, 0, 0);"></span>
							</a>
							<% } else { %>
							...
							<% } %>
							<a href="javascript:void(0)" style="float: right;" class="edit_lvl" onclick="editGoal('<%=goalid %>')" title="Edit Goal"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>
							</a>
							<a href="javascript:void(0)" onclick="getGoalApprovalStatus('<%=goalid %>','<%=gInnerList.get(3) %>','16');" title="View Workflow" style="float: right;">
								<i class="fa fa-eye" aria-hidden="true"></i>
							</a>
							
							<% } else { %>
							<a href="javascript:void(0);" style="float: right;" onclick="closeGoal('<%=goalid %>', 'view');" title="Close Goal Reason"><i class="fa fa-comment-o" aria-hidden="true"></i></a>
							<a href="javascript:void(0);" style="float: right;" onclick="openGoalForLive('<%=goalid%>','open');" title="Open Goal for Live"> <i class="fa fa-reply" aria-hidden="true"></i> </a>
							<%} %>
							<% } 
			                                               }%>
						...
						<div class="box-tools pull-right">
							<button class="btn btn-box-tool" data-widget="collapse" style="padding-right: 0px ! important; margin-right: -5px;"><i class="fa fa-plus"></i></button>
							<button class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
						</div>
					</div>
					<!-- /.box-header -->
					<div class="box-body"
						...
						<div style="float: left; width: 100%; margin-bottom: 5px;">
							<div style="float: left; width: 100%;">
								<div style="float: left; width: 100%;">
									<div style="float: right; margin-top: 5px;">
										<div id="starPrimaryG<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=goalCount %>" style="float: left;"></div>
										<div style="float: left; margin-right: 10px;">
											<b>Rated Score:</b>
											<%=uF.formatIntoOneDecimalWithOutComma(uF.parseToDouble(dblWeightScore+"")) %>/<%=uF.parseToDouble(gInnerList.get(16)) %></div>
										<div style="float: right; margin: 10px 25px 0px 0px; padding-bottom: 3px; padding-right: 3px;">
											<%
											...
											<table cellspacing="0" cellpadding="2" class="table table-bordered">
												<tr>
													<th colspan="4" style="padding: 0px;">Finalisation Score</th>
												</tr>
												<tr>
													<th style="width: 70px; padding: 0px;">Weightage</th>
													<th style="width: 100px; padding: 0px;">Actual Achieved</th>
													<th style="width: 105px; padding: 0px;">Achieved Share</th>
												</tr>
							...
							<% 
                                ...
                                %>
							<ul style="float: left; margin: 0px 10px; width: 100%; padding-left: 0px;">
								<li style="float: left; width: 100%; margin-top: 10px; margin-bottom: 5px;"> <!-- border-bottom: 1px solid #CCCCCC;  -->
									<div style="float: left; width: 100%;">
										<div style="float: left; margin: 0px 0px 0px 15px">
											<span class="<%=innerList.get(10) %>" style="margin: 0px 0px 0px 15px; float: left;"><strong>KRA:</strong>&nbsp;<%=uF.showData(innerList.get(2), "-") %></span>
										</div>
										<div style="float: right;">
											<div id="starPrimaryGK<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11)%>_<%=j%>" style="float: left;"></div>
											<div style="float: left;">
												<b>Rated Score:</b>
												<%=uF.formatIntoOneDecimalIfDecimalValIsThere(uF.parseToDouble(dblKRAWeightScore+"")) %>/<%=uF.parseToDouble(innerList.get(27)) %></div>
										</div>
										<script type="text/javascript">
									...
									<div style="float: left; width: 100%;">
										...
										<div style="float: left; width: 100%;">
											<div
												style="float: left; margin: 20px 5px 3px 55px; line-height: 15px; width: 25%;"><%=taskInnerList.get(1) %></div>
											<div
												id="KTProBarDiv_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"
											...
											<div style="float: left; margin: 20px 5px 3px 15px; width: 15%;">
												<input type="text" name="completedPercent_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"
													...
													style="width: 40px !important;" onkeypress="return isNumberKey(event)" /> 
													<a onclick="updateCompletedPercent('<%=strSessionEmpId %>', '<%=goalid %>', '<%=goalFreqId %>', '<%=innerList.get(11) %>','<%=taskInnerList.get(0) %>', 'KRA');"
													href="javascript:void(0);" title="Update"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>
												</a> <br />
												<span id="completedPercentStatusSpan_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"></span>
											</div>
											<% } else { %>
											<div style="float: left; margin: 20px 5px 3px 15px; width: 15%;">
												<label style="width: 40px;"><%=uF.showData(uF.formatIntoOneDecimalIfDecimalValIsThere(uF.parseToDouble(taskStatusPercent)), "") %></label>
												<br />
												<span id="completedPercentStatusSpan_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"></span>
											</div>
											<% } %>

											<div id="GivenTaskRatingDiv_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; padding-left: 10px;">
												<div id="starPrimaryGKT<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; margin: 20px 0px 0px 0px;"></div>
												<% if(commentCnt > 0 || uF.parseToInt(strUserTaskCnt) > 0) { %>
												<div style="margin-left: 25px;">
													<a href="javascript:void(0);" onclick="viewManagerAndHRComments('<%=strSessionEmpId %>', '<%=goalid %>', '<%=goalFreqId %>', '<%=innerList.get(11) %>', '<%=taskInnerList.get(0) %>', 'KRA')">Comments <%=(commentCnt + uF.parseToInt(strUserTaskCnt)) %></a>
												</div>
												<% } %>
											</div>
											<div id="TaskRatingDiv_<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="display: <%=hrMngrReview %>; float: left; padding-left: 10px;"> <!-- border-left: 1px solid #CCCCCC; -->
												<input type="hidden" name="hideGKTRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" id="hideGKTRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>">
												<div id="starGKTRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; margin: 5px 0px; width: 110px;"></div>
												<br />
												<div style="float: left; margin: 0px 0px 5px 0px;">
													<textarea rows="1" cols="40" name="strComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" id="strComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"></textarea>
												</div>
												<div style="float: left; margin: 0px 0px 5px 7px;">
													<a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=strSessionEmpId %>', '<%=goalid%>', '<%=goalFreqId%>', '<%=innerList.get(11) %>', '<%=taskInnerList.get(0) %>', 'KRA','Myself')">
														<input type="button" class="btn btn-primary" name="update" style="padding-bottom: 3px; padding-top: 3px;" value="Update">
													</a>
												</div>
											</div>
											<script type="text/javascript">
							...
							<ul
								...
								<li style="float: left; width: 100%; margin-bottom: 5px; margin-top: 10px;"> <!-- border-bottom: 1px solid #CCCCCC;  -->
									<div style="float: left; width: 100%;">
										<%  
										...
										<div class="row row_without_margin">
											<div class="col-lg-3 col-md-3" style="padding-left: 0px;">
												<div id="<%=i %>ProBarDiv<%=goalCount %>">
													...
													<div class="anaAttrib1">
														<span
															style="margin-left:<%=uF.parseToDouble(twoDeciTotProgressAvg) > 97 ? uF.parseToDouble(twoDeciTotProgressAvg)-6 : uF.parseToDouble(twoDeciTotProgressAvg)-3 %>%;"><%=uF.formatIntoOneDecimalIfDecimalValIsThere(uF.parseToDouble(twoDeciTot))%>%</span>
													</div>
													<div id="outbox">
											...
											<div class="col-lg-3 col-md-3" style="padding: 0px;">
												<div style="float: left; margin: 7px 0px 0px 16px;">
													<div id="starPrimaryGTarget<%=strSessionEmpId%>_<%=goalid %>_<%=goalFreqId %>"></div>
													<% if(commentCnt > 0 || uF.parseToInt(strUserCnt) > 0) { %>
													<div>
														<a href="javascript:void(0);" onclick="viewManagerAndHRComments('<%=strSessionEmpId %>', '<%=goalid %>', '<%=goalFreqId %>', '0', '0', 'TARGET')">Comments <%=(commentCnt + uF.parseToInt(strUserCnt)) %></a>
													</div>
													<% } %>
													<script type="text/javascript">
										...
										<div class="row row_without_margin">
											<div class="col-lg-12 col-md-12">
												<div id="TargetRatingDiv_<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" style="display: <%=hrMngrReview %>; float: left; padding-left: 10px;"> <!-- border-left: 1px solid #CCCCCC; -->
													<input type="hidden" name="hideGTargetRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" id="hideGTargetRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>">
													<div id="starGTargetRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; margin: 5px 0px; width: 110px;"></div>
													<br />
													<div style="float: left; margin: 0px 0px 5px 0px;">
														<textarea rows="1" cols="40" name="strTargetComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" id="strTargetComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>"></textarea>
													</div>
													<div style="float: left; margin: 0px 0px 5px 7px;">
														<a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=strSessionEmpId %>','<%=goalid%>', '<%=goalFreqId%>', '0', '0', 'TARGET', 'Myself')">
															<input type="button" class="btn btn-primary" name="update" style="padding-bottom: 3px; padding-top: 3px;" value="Update">
														</a>
													</div>
													<script type="text/javascript">
							...
							<ul style="float: left; width: 98%; padding-left: 0px;">
								<li style="float: left; width: 100%; margin-bottom: 5px; margin-top: 10px;"> <!-- border-bottom: 1px solid #CCCCCC;  -->
									<div style="float: left; width: 100%;">
										...
										<div
	                                        ...
	                                        %>
	                                           ...
	                                           <input type="text"
												...
												value="<%=uF.showData(goalStatusPercent, "") %>"
												style="width: 40px !important;"
												onkeypress="return isNumberKey(event)" readonly="readonly"/> <%-- <a
												onclick="updateCompletedPercent('<%=strSessionEmpId %>','<%=goalid %>', '<%=goalFreqId %>', '0' ,'0', 'GOAL');"
												href="javascript:void(0);" title="Update"><i
												class="fa fa-pencil-square-o" aria-hidden="true"></i>
											</a> --%> <br />
											...
											<%} %>
											<span id="completedPercentStatusSpan_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>"></span>
										</div>
										<% } else { %>
										<div
											style="float: left; margin: 20px 5px 3px 15px; width: 15%;">
										...
										<% } %>
										<div id="GivenGoalRatingDiv_<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>" style="float: left; padding-left: 10px;">
											<div id="starPrimaryGoal<%=strSessionEmpId %>_<%=goalid %>_<%=goalFreqId %>" style="float: left; margin: 20px 0px 0px 0px;"></div>
											<% if(commentCnt > 0 || uF.parseToInt(strUserCnt) > 0) { %>
											<div style="margin-left: 25px;">
												<a href="javascript:void(0);" onclick="viewManagerAndHRComments('<%=strSessionEmpId %>', '<%=goalid %>', '<%=goalFreqId %>', '0', '0', 'TARGET')">Comments <%=(commentCnt + uF.parseToInt(strUserCnt)) %></a>
											</div>
										...
										<div id="GoalRatingDiv_<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" style="display: <%=hrMngrReview %>; float: left; padding-left: 10px;"> <!-- border-left: 1px solid #CCCCCC; -->
											<input type="hidden" name="hideGoalRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" id="hideGoalRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>">
											<div id="starGoalRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; margin: 5px 0px; width: 110px;"></div>
											<br />
											<div style="float: left; margin: 0px 0px 5px 0px;">
											<!-- Created By Dattatray Date:07-09-21 -->
											<%
											if(!uF.parseToBoolean(gInnerList.get(12)) && uF.parseToInt(gInnerList.get(18)) == IConstants.PERSONAL_GOAL) {
	                                           if(uF.parseToInt(approveStatus)==1){//Created By Dattatray Date:08-09-21 Note : condition Chnaged
	                                        %>
												<textarea rows="1" cols="40" name="strGoalComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" id="strGoalComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>"></textarea>
											<%}else{ %>
												<textarea rows="1" cols="40" name="strGoalComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" id="strGoalComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" readonly="readonly"></textarea>
											<% } %>
											<%}else{ %>
												<textarea rows="1" cols="40" name="strGoalComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>" id="strGoalComment<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>"></textarea>
											<%} %>
											...
											<div style="float: left; margin: 0px 0px 5px 7px;">
												<a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=strSessionEmpId %>', '<%=goalid%>', '<%=goalFreqId%>', '0', '0', 'GOAL', 'Myself')">
													<!-- Created By Dattatray Date:07-09-21 -->
													<%
											if(!uF.parseToBoolean(gInnerList.get(12)) && uF.parseToInt(gInnerList.get(18)) == IConstants.PERSONAL_GOAL) {
	                                           if(uF.parseToInt(approveStatus)==1){//Created By Dattatray Date:08-09-21 Note : condition Chnaged
	                                        %>
													<input type="button" class="btn btn-primary" name="update" style="padding-top: 3px; padding-bottom: 3px;" value="Update">
												<% }else{ %>
												<input type="button" class="btn btn-primary" name="update" style="padding-top: 3px; padding-bottom: 3px;" value="Update" disabled="disabled">
												<% } %>
												<% }else{ %>
												<input type="button" class="btn btn-primary" name="update" style="padding-top: 3px; padding-bottom: 3px;" value="Update">
												<% } %>
										...
										<script type="text/javascript">
                                             $('#starPrimaryGoal<%=strSessionEmpId%>_<%=goalid %>_<%=goalFreqId %>').raty({
                                           	...
                                           	$('#starGoalRating<%=strSessionEmpId %>_<%=goalid%>_<%=goalFreqId%>').raty({
                                           		readOnly: <%=isReadOnly%>,/* Created By Dattatray Date:07-09-21 */
                                           		start: 0,
                                           		half: true,
                                           		targetType: 'number',
                                           		click: function(score, evt) {
				...
				<div class="box-header with-border">
					<h3 class="box-title" style="font-size: 14px !important;">Goal Forms(Others)</h3>
				</div>
				<!-- <h3 style="margin-top: 0px; font-size: 14px !important; font-weight: 600;" class="pagetitle">Goal Forms(Others)</h3> -->
				<%
                                    i=0;
				...
				<div class="box box-primary collapsed-box"
					...
					<div class="box-header with-border" style="height: 40px;">
						...
						<div class="box-tools pull-right">
							...
							<button class="btn btn-box-tool" data-widget="collapse" style="padding-right: 0px ! important; margin-right: -5px;">
					...
					<div class="box-body"
											...
											<div style="float: left; width: 100%;">
												<div style="float: left; width: 100%;">
													...
													<div style="float: right; width: 50%;">
														...
														<div style="float: left;">
															<b>Rated Score:</b>
															<%=uF.formatIntoOneDecimalIfDecimalValIsThere(uF.parseToDouble(dblWeightScore+"")) %>/<%=uF.parseToDouble(gInnerList.get(16)) %></div>
												...
												<div style="float: left; width: 100%;">
													...
													<div style="float: left; width: 100%;">
														...
														<div style="float: left; margin: 20px 5px 3px 5px; width: 7%;">
															<%=uF.showData(uF.formatIntoOneDecimalIfDecimalValIsThere(uF.parseToDouble(taskStatusPercent)), "0") %>%
														</div>
														<div id="GivenTaskRatingDiv_<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; padding-left: 10px;">
															<div id="starPrimaryGKT<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; margin: 20px 0px 0px 25px;"></div>
															<% if(commentCnt > 0 || uF.parseToInt(strUserTaskCnt) > 0) { %>
															<div style="margin-left: 25px;">
																<a href="javascript:void(0);" onclick="viewManagerAndHRComments('<%=empList.get(i) %>', '<%=goalid%>', '<%=goalFreqId%>', '<%=innerList.get(11) %>', '<%=taskInnerList.get(0) %>', 'KRA')">Comments <%=(commentCnt + uF.parseToInt(strUserTaskCnt)) %></a>
															</div>
															<% } %>
															<script type="text/javascript">
														...
														<div id="TaskRatingDiv_<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"
															style="display: <%=hrMngrReview %>; float: left; padding-left: 10px;"> <!-- border-left: 1px solid #CCCCCC; -->
															<input type="hidden" name="hideGKTRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" id="hideGKTRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>">
															<div id="starGKTRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; margin: 5px 0px; width: 110px;"></div>
															<br />
															<div style="float: left; margin: 0px 0px 5px 0px;">
																<textarea rows="1" cols="40" name="strComment<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" id="strComment<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" class="form-control autoWidth"></textarea>
															</div>
															<div style="float: left; margin: 0px 0px 5px 7px;">
																<a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=empList.get(i) %>', '<%=goalid%>', '<%=goalFreqId%>', '<%=innerList.get(11) %>', '<%=taskInnerList.get(0) %>', 'KRA','<%=strRole %>')">
																	<input type="button" class="btn btn-primary" name="update" style="padding-bottom: 3px; padding-top: 3px;" value="Update">
																</a>
															</div>
															<script type="text/javascript">
												...
												<div
													...
													<div class="row row_without_margin">
														...
														<div class="col-lg-4 col-md-4">
															<div>
																<div id="starPrimaryGTarget<%=empList.get(i)%>_<%=goalid%>_<%=goalFreqId%>"></div>
																<% if(commentCnt > 0 || uF.parseToInt(strUserCnt) > 0) { %>
																<div>
																	<a href="javascript:void(0);" onclick="viewManagerAndHRComments('<%=empList.get(i) %>', '<%=goalid %>', '<%=goalFreqId %>', '0', '0', 'TARGET')">Comments <%=(commentCnt + uF.parseToInt(strUserCnt)) %></a>
																</div>
																<% } %>
																<script type="text/javascript">
													...
													<% if((uF.parseToInt(hmHrIds.get(empList.get(i))) == uF.parseToInt(strSessionEmpId) && strSessionUserType != null && strSessionUserType.equals(IConstants.HRMANAGER)) || (strSessionUserType != null && !strSessionUserType.equals(IConstants.HRMANAGER))) { %>
													<div id="TargetRatingDiv_<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="margin-top: 10px;display: <%=hrMngrReview %>; float: left; padding-left: 10px;"> <!-- border-left: 1px solid #CCCCCC; -->
														<input type="hidden" name="hideGTargetRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" id="hideGTargetRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>">
														<div id="starGTargetRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; margin: 5px 0px; width: 110px;"></div>
														<br />
														<div style="float: left; margin: 0px 0px 5px 0px;">
															<textarea rows="1" cols="40" name="strTargetComment<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" id="strTargetComment<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" class="form-control autoWidth"></textarea>
														</div>
														<div style="float: left; margin: 0px 0px 5px 7px;">
															<a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=empList.get(i) %>','<%=goalid%>', '<%=goalFreqId%>', '0', '0', 'TARGET', '<%=strRole %>')">
																<input type="button" class="btn btn-primary" name="update" style="padding-bottom: 3px; padding-top: 3px;" value="Update">
															</a>
														</div>
														<script type="text/javascript">
												...
												<div style="float: left; width: 100%;">
													<div style="float: left; margin: 20px 0px 3px 0px; line-height: 15px; width: 21.5%;">
														...
														<span style="font-style: italic;"><a href="javascript:void(0);" style="font-size: 10px; font-weight: normal;"
															onclick="viewPerformanceAllowanceData('<%=gInnerList.get(1) %>', 'GT');">This Goal has Performance Incentives attached</a>
														</span>
														<% } else { %>
														<span style="font-size: 10px; font-style: italic; color: gray;">This Goal has no Performance Incentives attached</span>
														<% } %>
													</div>
													<div id="GoalProBarDiv_<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; margin: 3px 25px 3px 25px; width: 10%;">
														<div class="anaAttrib1">
															<span style="margin-left:<%=uF.parseToDouble(goalStatusPercent) > 95 ? uF.parseToDouble(goalStatusPercent) - 10 : uF.parseToDouble(goalStatusPercent) - 4%>%;"><%=uF.showData(uF.formatIntoOneDecimalIfDecimalValIsThere(uF.parseToDouble(goalStatusPercent)), "0")%>%</span>
														</div>
														<div id="outbox">
													...
													<div style="float: left; margin: 20px 5px 3px 15px; width: 7%;">
														<%=uF.showData(goalStatusPercent, "") %>% <input type="text" name="completedPercent_<%=empList.get(i) %>_<%=gInnerList.get(1) %>" id="completedPercent_<%=empList.get(i) %>_<%=gInnerList.get(1) %>"
															value="<%=uF.showData(goalStatusPercent, "") %>" style="width: 40px !important;" onkeypress="return isNumberKey(event)" /> <a onclick="updateCompletedPercent('<%=empList.get(i) %>','<%=gInnerList.get(1) %>','0', 'GOAL');" href="javascript:void(0);" title="Update">
															<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
														</a> <br />
														<span id="completedPercentStatusSpan_<%=empList.get(i) %>_<%=gInnerList.get(1) %>"></span>
													</div>
													<div id="GivenGoalRatingDiv_<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; padding-left: 10px;">
														<div id="starPrimaryGoal<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; margin: 20px 0px 0px 25px;"></div>
														<% if(commentCnt > 0 || uF.parseToInt(strUserCnt) > 0) { %>
														<div style="margin-left: 25px;">
															<a href="javascript:void(0);" onclick="viewManagerAndHRComments('<%=empList.get(i) %>', '<%=goalid %>', '<%=goalFreqId %>', '0', '0', 'GOAL')">Comments <%=(commentCnt + uF.parseToInt(strUserCnt)) %></a>
														</div>
														<% } %>
														<script type="text/javascript">
													...
													<% if((uF.parseToInt(hmHrIds.get(empList.get(i))) == uF.parseToInt(strSessionEmpId) && strSessionUserType != null && strSessionUserType.equals(IConstants.HRMANAGER)) || (strSessionUserType != null && !strSessionUserType.equals(IConstants.HRMANAGER))) { %>
													<div id="GoalRatingDiv_<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="display: <%=hrMngrReview %>; float: left; padding-left: 10px; "> <!-- border-left: 1px solid #CCCCCC; -->
														<input type="hidden" name="hideGoalRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" id="hideGoalRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>">
														<div id="starGoalRating<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" style="float: left; margin: 5px 0px; width: 110px;"></div>
														<br />
														<div style="float: left; margin: 0px 0px 5px 0px;">
															<textarea rows="1" cols="40" name="strGoalComment<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" id="strGoalComment<%=empList.get(i) %>_<%=goalid%>_<%=goalFreqId%>" class="form-control autoWidth"></textarea>
														</div>
														<div style="float: left; margin: 0px 0px 5px 7px;">
															<a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=empList.get(i) %>', '<%=goalid%>', '<%=goalFreqId%>', '0', '0', 'GOAL', '<%=strRole %>')">
																<input type="button" class="btn btn-primary" name="update" style="padding-bottom: 3px; padding-top: 3px;" value="Update">
															</a>
														</div>
														<script type="text/javascript">
						...
						<div class="nodata" style="border-radius: 4px 4px 4px 4px; padding: 10px; width: 95%; margin: 5px 5px 5px 5px">
							No Goal, KRA, Target assigned to <%=empname %>.
						</div>
						<% } %>
					</div>
					<!-- /.box-body -->
...
<script>
    ...
    $(window).bind('mousewheel DOMMouseScroll', function(event){
	    ...
	    if (event.originalEvent.wheelDelta > 0 || event.originalEvent.detail < 0) {
	        // scroll up
	        if($(window).scrollTop() == 0 && $("#sectionKra").scrollTop() != 0){
	        	$("#sectionKra").scrollTop($("#sectionKra").scrollTop() - 30);
	        }
			...
			if($(window).scrollTop() == 0 && $("#sectionFinalReview").scrollTop() != 0){
	        	$("#sectionFinalReview").scrollTop($("#sectionFinalReview").scrollTop() - 30);
	        }
	    }
	    else {
	        // scroll down
   ...
   $(window).keydown(function(event){
   		...
   		{
   			if($(window).scrollTop() + $(window).height() > $(document).height() - 15) {
	    		   $("#sectionKra").scrollTop($("#sectionKra").scrollTop() + 50);
	   			}
			...
			if($(window).scrollTop() + $(window).height() > $(document).height() - 15) {
	    		   $("#sectionFinalReview").scrollTop($("#sectionFinalReview").scrollTop() + 50);
	   			}
   		}
   ...
   /* Created by Dattatray */
	function giveRemark() {
   		...
   		$.ajax({
   			url : "AnnualAppraisalReport.action",
   			cache : false,
   			success : function(data) {
   				$(dialogEdit).html(data);
   			}
