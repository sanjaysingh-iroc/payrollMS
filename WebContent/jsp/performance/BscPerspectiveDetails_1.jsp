...
<%
 UtilityFunctions uF = new UtilityFunctions();
	...
	Map<String,List<String>> hmPerspectiveDetails = (Map<String,List<String>>) request.getAttribute("bscperspectiveDetails");
	%>

<section class="col-lg-4 connectedSortable" style="width: 100%;">
		<div class="box box-primary" style="border-top-color: #cda55f /*#E0E0E0;*/">
			<% if (hmPerspectiveDetails != null && hmPerspectiveDetails.size()>0) { %>
			...
			<%	
				 if (hmPerspectiveDetails != null && hmPerspectiveDetails.size()>0) {
					...
					while (itPerspective.hasNext()) {
						...
						List<String> perspectiveList = hmPerspectiveDetails.get(perspective);
						String perspectiveName = perspectiveList.get(0);
						String perspectiveColor = perspectiveList.get(1);
						List<String> goalList = hmGoal1.get(perspective);
						int count1 = goalList.size();
						//int count1 = 0;
						int height = 0;
						if(goalList != null && goalList.size() > 0) { %>
							<div id = "maindiv" style = "float:left;">
							<div align="center" id = "perspectId_<%=perspective%>" style="float: left; width: 3%; word-wrap:break-word; margin: 20px 10px 10px 0px; padding: 50px 17px 0px 10px; border-radius:5px; background-color:<%=perspectiveColor%>">
               				 <%=perspectiveName%>
							</div>
	                    	...
	                    	int goalCount = 0;
                  			Map<String, List<List<String>>> hmGoalKra = hmGoalKraSuperIdwise.get(perspective);
                  		 	if(hmGoalKra == null) hmGoalKra = new HashMap<String, List<List<String>>>();
                     		Iterator<String> itgKRA = hmGoalDetails.keySet().iterator();
                  			 while(itgKRA.hasNext()) {	
                      			goalCount++;
                      		//	count1++;
                      			String goalAndFreqId = itgKRA.next();
                      			if(goalAndFreqId.contains(goal1)){
                                        ...
                                        <%
                                         if(gInnerList.get(20) != null && gInnerList.get(20).equals("KRA")) {
                                             ...
                                             double totalKraStatus =0;
                                            	double kraStatus = 0;
                                             totalKraStatus = uF.parseToDouble(hmKraCompletedPercentage.get(perspective+"_"+goalid+"_"+goalFreqId+"_PERCENTAGE"));
                                             kraTotalTaskCount= uF.parseToInt(hmKraCompletedPercentage.get(perspective+"_"+goalid+"_"+goalFreqId+"_TASKCOUNT")); 
						...
						<% }%>
								 ...
								 <div style="float: left; width: 40%; margin-top: 5px;">                                                  
                                              ...
                                              List<List<String>> taskOuterList = hmKRATasks.get(perspective+"_"+goalid+"_"+goalFreqId+"_"+innerList.get(11));
                                             ...
                                             <div  style="float: left; width: 100%;">
											  ...
											  String hrMngrReview = "block";
                                                  ...
                                                  //       System.out.println("avgRating::"+avgRating);
                                                         ...
                                                         if(empComment != null && !empComment.equals("null")) {
                                                          	commentCnt++;
                                                          }
                                                         %>
                                                         <div style="float: left; width: 100%;">
												 ...
												 <div id="KTProBarDiv_<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; width: 30%;">                                                                   
													<div class="anaAttrib1">
                                                         ...
                                                         <%
                                                             ...
                                                             %>
                                                            <%-- <div >
                                                                <input type="text" name="completedPercent_<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" id="completedPercent_<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" value="<%=uF.showData(taskStatusPercent, "") %>" style="width: 30px !important; margin-left:18px; margin-top:10px;" onkeypress="return isNumberKey(event)" />
                                                                 <a onclick="updateCompletedPercent('<%=perspective %>', '<%=goalid %>', '<%=goalFreqId %>', '<%=innerList.get(11) %>','<%=taskInnerList.get(0) %>', 'KRA','<%=accessFlag %>');" href="javascript:void(0);" title="Update">
													...
													<span id="completedPercentStatusSpan_<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"></span>                                                               
												 		...
												 		<div id="GivenTaskRatingDiv_<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; width:100%">
                                                               ...
                                                               <% 
                                                               	...
                                                               	if(currUserType != null && currUserType.equals(strBaseUserType)) {
                                                             		...
                                                             		%>
													 			<%-- <a style="height: 1px; width: 2px; padding-top:2px;" href="javascript:void(0)" onclick="updateComment1('','<%=goalid %>', '<%=goalFreqId %>', '<%=innerListdata %>','<%=taskInnerList.get(0) %>', 'KRA','<%=accessFlag %>')"><i class="fa fa-plus-circle" aria-hidden="true"></i>Add Comment</a> --%>
													 		</div>
                                                         ...
                                                         <% 
                                                             ...
                                                             <% if(dataType == null || dataType.equals("L")) {%>	
                                                             <%-- <div id="TaskRatingDiv_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" style="float: left; padding-left: 10px; border-left: 1px solid #CCCCCC;">
                                                                 <script type="text/javascript">
                                                                     $(function() {
                                                                     	$('#starGKTRating_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>').raty({
                                                                     		readOnly: false,
                                                                     		start: 0,
                                                                     		half: true,
                                                                     		targetType: 'number',
                                                                     		click: function(score, evt) {
                                                                     				$('#hideGKTRating<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>').val(score);
                                                                     }
                                                                     });
                                                                     });
                                                                 </script>
                                                                 <input type="hidden"
                                                                     name="hideGKTRating<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"
                                                                     id="hideGKTRating<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>">
                                                                 <div
                                                                     id="starGKTRating<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"
                                                                     style="float: left; margin: -4px 0px 0px 25px; width: 110px;"></div>
                                                                 <br />
                                                                 <div style="float: left; margin: 0px 0px 5px 0px;">
                                                                     <textarea rows="1" cols="40" name="strComment<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>" id="strComment<%=perspective %>_<%=goalid %>_<%=goalFreqId %>_<%=innerList.get(11) %>_<%=taskInnerList.get(0) %>"></textarea>
                                                                 </div>
                                                                 <div style="float: left; margin: 0px 0px 5px 7px;">
                                                                     <a href="javascript:void(0);" onclick="updateKRATaskRatingAndComment('<%=perspective %>','<%=goalid %>', '<%=goalFreqId %>', '<%=innerList.get(11) %>','<%=taskInnerList.get(0) %>', 'KRA','<%=feedbackUserType %>','<%=dataType%>','<%=currUserType%>')">
							...
							<script type="text/javascript">
								$(function() {
									var count = "<%=count1 %>";
									document.getElementById("perspectId_<%=perspective%>").style.height =((300 * count) +"px");
								});
							</script>
							<% } else { %>
