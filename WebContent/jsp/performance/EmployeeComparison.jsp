...
function getEmpComparisonData(dataType) {
	...
	function submitForm() {
 	...
 	function callEmployeeComparisonData() {
		...
		$("input:checkbox[name=attribParam]:checked").each(function() {
			attribParam.push($(this).val());
		});
		...
		$("#empComparisonData").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		var action = 'EmployeeComparisonData.action?attribParam=' + attribParam + '&dateParam=' + datedParam + '&wLocParam=' + wLocParam 
		...
		//alert("action ===> " + action);
		getContent('empComparisonData', action);
		
		if(attribCheck == 'AT') {
			setCheckedAndUnchekedAttribute('T');
		} else {
			setCheckedAndUnchekedAttribute('F');
		}
	}
 	
 	
 	function setCheckedAndUnchekedAttribute(status) {
 		//alert("status ===>> " + status);
 		var  arr= document.getElementsByName("attribParam");
 		if(status=='T') {
 			//$('.chkAttrib').attr( 'checked', 'checked');
 	   		for(i=0;i<arr.length;i++){ 
 	   	  		arr[i].checked=true;
 	   	 	}
 		} else {
 			//$('.chkAttrib').attr( 'checked', '');
 			for(i=0;i<arr.length;i++){ 
 	   	  		arr[i].checked=false;
 	   	 	}
 		}
		...
		<div style="width: 100%;">
			<ul class="nav nav-pills">
				<li class="<%=(dataType == null || dataType.equals("MYTEAM")) ? "active" : "" %>" style="border-radius: 10px 10px 0px 0px; <%=(dataType == null || dataType.equals("MYTEAM")) ? "" : "background-color: rgb(226, 226, 226)" %>;">
					<a href="javascript:void(0)" onclick="getEmpComparisonData('MYTEAM')" style="padding: 4px 15px !important; border-radius: 10px 10px 0px 0px;">My Team</a>
				</li>
				<li class="<%=(dataType == null || dataType.equals(strBaseUserType)) ? "active" : "" %>" style="border-radius: 10px 10px 0px 0px; <%=(dataType == null || dataType.equals(strBaseUserType)) ? "" : "background-color: rgb(226, 226, 226)" %>;">
					<a href="javascript:void(0)" onclick="getEmpComparisonData('<%=strBaseUserType %>')" style="padding: 4px 15px !important;border-radius: 10px 10px 0px 0px;"><%=strBaseUserType %></a>
				</li>
			</ul>
		</div>
	<% } %>
				...
				<div class="box-body" style="padding: 5px; overflow-y: auto;min-height: 600px;">
	                ...
	                <div id="MainDiv" class="row" style="margin-left: 0px;margin-right: 0px;"> 
						...
						<div class="col-lg-3 col-md-3 col-sm-12" style="padding: 2px; overflow-y: hidden; max-height: 500px;" id="leftSectionTeamComparison"><!-- @uther dattatray -->
					                ...
					                <div class="box-body" style="padding: 5px; overflow-y: auto;">
					                    <div class="content1" id="idlocation">
											<table>
												...
												<tr>
													<td class="textblue"><input type="checkbox" value="<%=(String) ((FillWLocation) wLocationList.get(i)).getwLocationId()%>" name="checkLocation" id="checkLocation" checked="checked" onclick="callEmployeeComparisonData();" /></td>
					                ...
					                <div class="box-body" style="padding: 5px; overflow-y: auto;">
					                    <div class="content1" id="iddepart">
											<table>
												...
												<tr>
													<td class="textblue"><input type="checkbox" value="<%=(String) ((FillDepartment) departmentList.get(i)).getDeptId()%>" name="checkDepart" checked="checked" onclick="callEmployeeComparisonData();" /></td>
					                ...
					                <div class="box-body" style="padding: 5px; overflow-y: auto;">
					                    <div class="content1" id="idlevels">
											<table>
												...
												<tr>
													<td class="textblue"><input type="checkbox" value="<%=(String) ((FillLevel) levelList.get(i)).getLevelId()%>" name="checkLevel" checked="checked" onclick="callEmployeeComparisonData();" />
													</td>
					            ...
					            <div class="box box-primary" style="border-top-color:#cda55f;/* #EEEEEE;*/">
					                ...
					                <div class="box-body" style="padding: 5px; overflow-y: auto;">
					                    <div class="content1" id="idMeasures">
											<table>
							                  	<tr>
								                  	<td class="textblue"><input type="checkbox" value="SKILL" name="filterParam" checked="checked" onclick="callEmployeeComparisonData();"/></td>
								                  	<td>Skills</td> 
							                  	</tr>
							                  	<tr>
								                  	<td class="textblue"><input type="checkbox" value="REVIEW" name="filterParam" checked="checked" onclick="callEmployeeComparisonData();"/></td>
								                  	<td>Review</td> 
							                  	</tr>
							                  	<tr>
								                  	<td class="textblue"><input type="checkbox" value="GOAL_KRA_TARGET" name="filterParam" checked="checked" onclick="callEmployeeComparisonData();"/></td>
								                  	<td>Goals/ BSC/ KRAs/ Targets</td> 
							                  	</tr>
							                  	<tr>
								                  	<td class="textblue"><input type="checkbox" value="AT" name="filterParam" checked="checked" onclick="callEmployeeComparisonData();"/></td>
								                  	<td>Attribute</td> 
							                  	</tr>
							                  	<tr>
								                  	<td class="textblue"><input type="checkbox" value="LH" name="filterParam" checked="checked" onclick="callEmployeeComparisonData();"/></td>
								                  	<td>Logged Hours</td> 
							                  	</tr>
							                  	<% if((uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_SHOW_SALARY_IN_PROFILE_USERWISE)) && hmFeatureUserTypeId.get(IConstants.F_SHOW_SALARY_IN_PROFILE_USERWISE).contains(USERTYPEID)) || (!uF.parseToBoolean(hmFeatureStatus.get(IConstants.F_SHOW_SALARY_IN_PROFILE_USERWISE)) && hmFeatureUserTypeId != null && hmFeatureUserTypeId.get(IConstants.F_SHOW_SALARY_IN_PROFILE_USERWISE+"_USER_IDS") != null && hmFeatureUserTypeId.get(IConstants.F_SHOW_SALARY_IN_PROFILE_USERWISE+"_USER_IDS").contains(strSessionEmpId))) { %>
								                  	<tr>
									                  	<td class="textblue"><input type="checkbox" value="GC" name="filterParam" checked="checked" onclick="callEmployeeComparisonData();"/></td>
											...
											<ul style="padding-left: 0px;">
												...
												<li><strong><%=innerList.get(1)%></strong>
													<ul>
														...
														%>
														<li><input type="checkbox" value="<%=attributeList1.get(0)%>" class="chkAttrib" name="attribParam" checked="checked" onclick="callEmployeeComparisonData();" /> <%=attributeList1.get(1) %></li>
						...
						<div class="col-lg-9 col-md-9 col-sm-12" style="overflow-y: hidden;max-height: 500px;" id="rightSectionTeamComparison">
						<div id="empComparisonData" ></div>
						</div>
...
$(window).bind('mousewheel DOMMouseScroll', function(event){
    if (event.originalEvent.wheelDelta > 0 || event.originalEvent.detail < 0) {
        // scroll up
        if($(window).scrollTop() == 0 && $("#leftSectionTeamComparison").scrollTop() != 0) {
        	$("#leftSectionTeamComparison").scrollTop($("#leftSectionTeamComparison").scrollTop() - 30);
        }
        if($(window).scrollTop() == 0 && $("#rightSectionTeamComparison").scrollTop() != 0) {
        	$("#rightSectionTeamComparison").scrollTop($("#rightSectionTeamComparison").scrollTop() - 30);
        }
    } else {
        // scroll down
...
$(window).keydown(function(event){
	if(event.which == 40 || event.which == 34){
		if($(window).scrollTop() + $(window).height() > $(document).height() - 15) {
			$("#leftSectionTeamComparison").scrollTop($("#leftSectionTeamComparison").scrollTop() + 50);
   		}
		if($(window).scrollTop() + $(window).height() > $(document).height() - 15) {
			$("#rightSectionTeamComparison").scrollTop($("#rightSectionTeamComparison").scrollTop() + 50);
   		}
