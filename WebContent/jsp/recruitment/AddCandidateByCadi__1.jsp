...
jQuery(document).ready(function() {
    ...
    $("input[name=fatherDob]").datepicker({format: 'dd/mm/yyyy', yearRange: '1950:<%=currentYear%>', changeYear: true});
    $("input[name=motherDob]").datepicker({format: 'dd/mm/yyyy', yearRange: '1950:<%=currentYear%>', changeYear: true});
    $("input[name=spouseDob]").datepicker({format: 'dd/mm/yyyy', yearRange: '1950:<%=currentYear%>', changeYear: true});
    $("input[name=memberDob]").datepicker({format: 'dd/mm/yyyy', yearRange: '1950:<%=currentYear%>', changeYear: true});
...
function addLanguages() {
	...
	divTag.innerHTML = 	"<table class=\"table table_no_border\">" +
						...
						"<td width=\"50px\" align=\"center\"><a href=\"javascript:void(0)\" onclick=\"addLanguages()\" class=\"add-font\"></a></td>" +
						"<td><a href=\"javascript:void(0)\" onclick=\"removeLanguages(this.id)\" id=\""+cnt+"\" class=\"remove-font\" ></a></td>" +
						"</table>"; 
...
function addEducation() {
    ...
    divTag.innerHTML = "<table class=\"table table_no_border\"><tr><td><select name=\"degreeName\" style=\"width:110px;\" onchange=\"checkEducation(this.value,"+cnt+")\"> "+
...
function addSibling() {
    ...
    divTag.innerHTML = 	"<%=request.getAttribute("sbSibling")%>" +
    		...
    		+"<a href=\"javascript:void(0)\" onclick=\"removeSibling(this.id)\" id=\""+siblingcnt+"\" class=\"remove\" >Remove</a>"
    		+"<a href=javascript:void(0) onclick=addSibling() class=add>Add</a></span></td></tr>" +
            "</table>"; 
...
function addPrevEmployment() {
	...
	divTag.innerHTML = "<%=request.getAttribute("sbPrevEmployment")%>" + 
					"<tr><td colspan=\"2\" ><span style=\"float: right;\">"
    		+"<a href=\"javascript:void(0)\" onclick=\"removePrevEmployment(this.id)\" id=\""+cnt+"\" class=\"remove\" >Remove</a>"
    		+"<a href=javascript:void(0) onclick=addPrevEmployment() class=add>Add</a></span></td></tr>" +
    		/* "<td class=\"txtlabel alignRight\"><a href=\"javascript:void(0)\" onclick=\"removePrevEmployment(this.id)\" id=\""+cnt+"\" class=\"remove\" >&nbsp;</td>";
					"</tr>" + */ 
				     "</table>";
...
<ul class="tabs">
	...
	<%-- <s:if test="step==9 || mode=='report'">
		...
		<li><a href="#">Salary Structure Information of <%=uF.showData((String)request.getAttribute("CandidateName"), "") %></a></li>
		...
		<%-- 	
		 ...
		 --%>	
			...
			<tr id = "tr_ref_details" style="<%=displayRef%>;">
				<td class="txtlabel alignRight">Ref. Employee Id:<sup>*</sup></td>
				<td> 
...
<div>
	...
	<div id="div_education">
       ...
       <h3 style="padding:0px;margin:5px 0px 10px 0px">Enter Candidate educational details</h3>
		...
		<div id="row_education" class="row_education">  
                <table class="table table_no_border" style="font-size: 12px;">
				<tr><td width="120px"><label>Degree Name<%=validAsterix %></label></td>
				    <td width="120px"><label>Duration<%=validAsterixDD %></label></td>
				    <td width="120px"><label>Completion Year<%=validAsterixDCY %></label></td>
				    <td width="120px"><label>Grade<%=validAsterixDG %></label></td>
                </tr> 
                </table>   
		</div>
		<% 	
		...
		%>
				<div id="row_education<%=((ArrayList)alEducation.get(i)).get(0)%>" class="row_education">
                    <table class="table table_no_border">    
						<tr>
							<td>
							<select name="degreeName" class="<%=validReqOpt %>"  style="width: 120px !important;">
	...
	<h3 style="padding:0px;margin:5px 0px 10px 0px">Enter employee languages</h3>
           ...
           <div id="row_language" class="row_language">  
                <table class="table table_no_border" style="font-size: 12px">              
				  <tr><td width="190px"><label>Language Name<%=validAsterix %></label></td>
			...
			%>
				...
				<div id="row_language<%=i%>" class="row_language">
					<table class="table table_no_border">
						<tr><td><input type="text" class="<%=validReqOpt %>" style="height: 25px; width: 180px;" name="languageName" value="<%=((ArrayList)alLanguages.get(i)).get(1)%>" ></input></td>
			...
			<div id="col_prev_employer<%=i%>" style="float: left;">
             ...
             <table class="table table_no_border">
				...
				%>
			 	...
			 	<td><input type="text" name="prevCompanyName" style="height:25px; width: 180px;"  class="<%=validReqOpt %>" name="prevCompanyLocation" value="<%=((ArrayList)alPrevEmployment.get(i)).get(1)%>"></input></td>
			 	</tr>
		...
		<%}
			...
			<div id="col_prev_employer" style="float: left;">
              ...
              <table class="table table_no_border">
				...
				%>
			 	<td class="txtlabel alignRight"> Company Name:<%=validAsterix %></td>
			 	<td><input type="text" name="prevCompanyName" style="height:25px; width: 180px;" class="<%=validReqOpt %>" name="prevCompanyName" ></input></td></tr> 
						...
						%>
						  <tr>
						  		...
						  		<td class="txtlabel alignRight" style="text-align: -moz-center">
						  		     ...
						  		     %>
						  		     <input type="file" name="idDoc<%=i%>" id="idDoc<%=i%>" class="<%=validReqOpt %>" onchange="<%=onChangeFunct%>"/>
	                    ...
	                    <!-- ====end parvez on 01-07-2021 -->
		                    <tr>
						 	...
						 	<tr>
								...
								<td class="txtlabel alignRight"><input type="file" name="idDoc1" id="idDoc1" class="<%=validReqOpt %>" onchange="fillFileStatus('idDoc2Status')"/></td>
		                   	</tr>
						 	<tr>
								...
								<td class="txtlabel alignRight"><input type="file" name="idDoc2" name="idDoc2" class="<%=validReqOpt %>" onchange="fillFileStatus('idDoc3Status')"/></td>
					    	</tr>
					    	<!--  <tr><td>
					    		<a href="javascript:void(0)" onclick="addBackgroundVerificationDocuments(0);"><i class="fa fa-plus-circle"></i>Add More Documents...</a>&nbsp;&nbsp;
					    	</td>
