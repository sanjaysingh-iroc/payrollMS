...
function submitForm(type){
	document.frm_fromRecon.exportType.value='';
	...
	$.ajax({
		type : 'POST',
		url: 'ReconciliationReport.action?f_org='+org+paramValues,
...
function getSelectedValue(selectId) {
	...
	function generateReconciliationPdfReport() {
		/* var financialYear=document.frm_fromRecon.financialYear.value;
		var strMonth=document.frm_fromRecon.strMonth.value;
		var f_strWLocation=document.frm_fromRecon.f_strWLocation.value;
		var f_department=document.frm_fromRecon.f_department.value;
		var f_level=document.frm_fromRecon.f_level.value;
		var f_org=document.frm_fromRecon.f_org.value;
		//var f_salaryhead=document.frm_fromRecon.f_salaryhead.value;
		
		var salaryheadLen = document.getElementById("idsalaryhead").length;
		 var f_salaryhead = "";
		 for (var i = 0; i < salaryheadLen; i++) {
		   var isSelected = document.getElementById("idsalaryhead").options[i].selected;
		  if(isSelected = (isSelected)) {
		  	if(f_salaryhead == "") {
		  		f_salaryhead += document.getElementById("idsalaryhead").options[i].value;
		  	} else {
		  		f_salaryhead += "," + document.getElementById("idsalaryhead").options[i].value;
		  	}
		  }
		...
		var url='ReconciliationPdfReport.action?financialYear='+financialYear;
		url+="&strMonth="+strMonth;
		url+="&f_strWLocation="+f_strWLocation+"&f_department="+f_department;
		url+="&f_level="+f_level+"&f_org="+f_org;
		url+="&f_salaryhead="+f_salaryhead;
		
		window.location = url; */

		document.frm_fromRecon.exportType.value = 'pdf';
		document.frm_fromRecon.submit();
	}
...
<%-- <jsp:include page="../../common/SubHeader.jsp">
	<jsp:param value="Reconciliation Report" name="title"/>
</jsp:include> --%>
	...
	<div class="box-body" style="padding: 5px; overflow-y: auto; min-height: 600px;">
		<div class="box box-primary collapsed-box" style="border-top-color: #EEEEEE;">
			...
			<div class="box-body" style="padding: 5px; overflow-y: auto; display: none;">
				<s:form name="frm_fromRecon" action="ReconciliationReport" theme="simple">
		...
		<!-- <div class="col-md-2" style="margin: 0px 0px 10px 0px; float: right;">
			<a onclick="generateReconciliationPdfReport();" href="javascript:void(0)" class="fa fa-file-pdf-o"></a>
		</div>  -->
			
<div class="col-md-2 pull-right">
					
<a onclick="generateReconciliationPdfReport();" href="javascript:void(0)" style="background-repeat: no-repeat;float: right;"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
		...
		<display:table name="reportList" class="table table-bordered" id="lt1">
			...
			<display:column style="text-align:left;" valign="top" title="Organization Code"><%=((java.util.List) pageContext.getAttribute("lt1")).get(1)%></display:column>
			...
			<display:column style="text-align:left;" valign="top" title="Employee Code"><%=((java.util.List) pageContext.getAttribute("lt1")).get(3)%></display:column>
			<display:column style="text-align:left;" valign="top" nowrap="nowrap" title="Employee Name"><%=((java.util.List) pageContext.getAttribute("lt1")).get(4)%></display:column>
			<display:column style="text-align:left;" valign="top" nowrap="nowrap" title="Pan No"><%=((java.util.List) pageContext.getAttribute("lt1")).get(5)%></display:column>
			<display:column style="text-align:right;" valign="top" nowrap="nowrap" title="<%=uF.getMonth(uF.parseToInt(strPrevMonth))%>"><%=((java.util.List) pageContext.getAttribute("lt1")).get(6)%></display:column>
			<display:column style="text-align:right;" valign="top" nowrap="nowrap" title="<%=uF.getMonth(uF.parseToInt(strMonth))%>"><%=((java.util.List) pageContext.getAttribute("lt1")).get(7)%></display:column>
			<display:column style="text-align:right;" valign="top" title="Difference"><%=((java.util.List) pageContext.getAttribute("lt1")).get(8)%></display:column>
