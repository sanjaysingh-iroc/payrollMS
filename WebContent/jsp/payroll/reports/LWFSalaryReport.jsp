...
function submitForm(type){
	document.frm_fromLWF.exportType.value='';
	...
	$.ajax({
		type : 'POST',
		url: 'LWFSalaryReport.action?f_org='+org+paramValues,
...
function generateLWFSalaryReport(){
	document.frm_fromLWF.exportType.value='pdf';
	document.frm_fromLWF.submit();
}

function generateReportExcel(){
	document.frm_fromLWF.exportType.value='excel';
	document.frm_fromLWF.submit();
}
...
<%-- <jsp:include page="../../common/SubHeader.jsp">
	<jsp:param value="<%=IMessages.TReportLWFSalary%>" name="title"/>
</jsp:include> --%>
		...
		<div class="box-body" style="padding: 5px; overflow-y: auto; min-height: 600px;">
			<div class="desgn" style="margin-bottom: 5px;background:#f5f5f5; color:#232323;">
				<div class="box box-primary collapsed-box" style="border-top-color: #EEEEEE; margin-top: 10px;">
	                ...
	                <div class="box-body" style="padding: 5px; overflow-y: auto; display:none;">
						<s:form name="frm_fromLWF" action="LWFSalaryReport" theme="simple">
			...
			<!-- <div class="col-md-2" style="margin: 0px 0px 10px 0px; float: right;">
				<a onclick="generateLWFSalaryReport();" href="javascript:void(0)" class="fa fa-file-pdf-o" >&nbsp;&nbsp;</a>
			...
			<div class="col-md-2 pull-right">
				<a onclick="generateLWFSalaryReport();" href="javascript:void(0)" style="background-repeat: no-repeat;float: right;"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
				<a onclick="generateReportExcel();" href="javascript:void(0)" style="background-repeat: no-repeat;float: right;"><i class="fa fa-file-excel-o" aria-hidden="true"></i></a>
			</div>

			
			<%-- <div style="text-align:center;margin:10px" > <h2>LWF Salary Report for the month of <%=strMonthYear%> </h2></div> --%>
			...
			<display:table name="reportList" cellspacing="1" class="table table-bordered" id="lt1">
				...
				<display:column style="text-align:left;" valign="top" title="LWF Wages"><%=((java.util.List) pageContext.getAttribute("lt1")).get(2)%></display:column>
				<display:column style="text-align:left;" valign="top" title="Employee Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(3)%></display:column>
				<display:column style="text-align:left;" valign="top" title="Employer Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(4)%></display:column>
			</display:table>
			<% } else { 
				String title="";
			...
			%>
			<display:table name="reportList" cellspacing="1" class="table table-bordered" id="lt1">
				<display:column style="text-align:left;" valign="top" nowrap="nowrap" title="<%=title %>"><%=((java.util.List) pageContext.getAttribute("lt1")).get(0)%></display:column>	
				<display:column style="text-align:left;" valign="top" title="LWF Wages"><%=((java.util.List) pageContext.getAttribute("lt1")).get(1)%></display:column>
				<display:column style="text-align:left;" valign="top" title="Employee Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(2)%></display:column>
				<display:column style="text-align:left;" valign="top" title="Employer Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(3)%></display:column>
			</display:table>
			<% } %>
