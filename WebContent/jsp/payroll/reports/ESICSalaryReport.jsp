...
function submitForm(type){
	document.frm_fromESIC.exportType.value='';
	...
	if(type == '2') {
		...
		+'&financialYear='+financialYear+'&strMonth='+strMonth+'&f_level='+f_level+'&paramSelection='+paramSelection;
	}
	//alert("service ===>> " + service);
	$("#divResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
	$.ajax({
		type : 'POST',
		url: 'ESICSalaryReport.action?f_org='+org+paramValues,
...
function generateESICSalaryReport(){
	/* var financialYear=document.frm_fromESIC.financialYear.value;
	var strMonth=document.frm_fromESIC.strMonth.value;
	var f_strWLocation=document.frm_fromESIC.f_strWLocation.value;
	var f_department=document.frm_fromESIC.f_department.value;
	//var f_level=document.frm_fromESIC.f_level.value;
	var f_org=document.frm_fromESIC.f_org.value;
	var f_service=document.frm_fromESIC.f_service.value; 
	 
	var url='ESICSalaryPdfReports.action?financialYear='+financialYear;
	...
	window.location = url; */
	document.frm_fromESIC.exportType.value='pdf';
	document.frm_fromESIC.submit();
}
...
function generateReportExcel(){
	...
	var url='ESICSalaryExcelReports.action?financialYear='+financialYear;
	url+="&strMonth="+strMonth;
	url+="&f_strWLocation="+f_strWLocation+"&f_department="+f_department;
	//url+="&f_level="+f_level+"&f_org="+f_org;
	url+="&f_service="+f_service+"&f_org="+f_org;
	window.location = url; */
	document.frm_fromESIC.exportType.value='excel';
	document.frm_fromESIC.submit();
}
...
<%-- <jsp:include page="../../common/SubHeader.jsp">
	<jsp:param value="<%=IMessages.TReportESICSalary%>" name="title"/>
</jsp:include> --%>
		...
		<div class="desgn" style="margin-bottom: 5px;background:#f5f5f5; color:#232323;">							
			<div class="box box-primary collapsed-box" style="border-top-color: #EEEEEE; margin-top: 10px;">
                ...
                <div class="box-body" style="padding: 5px; overflow-y: auto; display:none;">
					<s:form name="frm_fromESIC" action="ESICSalaryReport" theme="simple">
						...
						<div class="row row_without_margin">
							...
							<div class="col-lg-11 col-md-11 col-sm-12 paddingleft0 paddingright0">
								...
								<div class="col-lg-2 col-md-6 col-sm-12 autoWidth paddingleftright5">
									...
									<s:select theme="simple" name="f_level" id="f_level" listKey="levelId" listValue="levelCodeName" list="levelList"
					 					key="" headerKey="" headerValue="Select Level" onchange="submitForm('2');"/>
								</div>
								<div class="col-lg-2 col-md-6 col-sm-12 autoWidth paddingleftright5">
	...
	<br>
		<!-- <div class="col-md-2" style="margin: 0px 0px 10px 0px; float: right;">
			<a onclick="generateESICSalaryReport();" href="javascript:void(0)" class="fa fa-file-pdf-o" >&nbsp;&nbsp;</a>
		 ...
		 <div class="col-md-2 pull-right">
			<a onclick="generateESICSalaryReport();" href="javascript:void(0)" style="background-repeat: no-repeat;float: right;"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
			<a onclick="generateReportExcel();" href="javascript:void(0)" style="background-repeat: no-repeat;float: right;"><i class="fa fa-file-excel-o" aria-hidden="true"></i></a>
		</div>
		 
		 
		 
		<%-- <div style="text-align:center;margin:10px" > <h2>ESIC Salary Report for the month of <%=strMonthYear%> </h2></div> --%>
		...
		<display:table name="reportList" cellspacing="1" class="table table-bordered" id="lt1">
			...
			<display:column style="text-align:center;" valign="top" title="ESIC Acc. No"><%=((java.util.List) pageContext.getAttribute("lt1")).get(3)%></display:column>	
			<display:column style="text-align:right;" valign="top" title="Paid Days"><%=((java.util.List) pageContext.getAttribute("lt1")).get(4)%></display:column>
			<display:column style="text-align:right;" valign="top" title="ESIC Wages"><%=((java.util.List) pageContext.getAttribute("lt1")).get(5)%></display:column>
			<display:column style="text-align:right;" valign="top" title="Employee Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(6)%></display:column>
			<display:column style="text-align:right;" valign="top" title="Employer Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(7)%></display:column>
		</display:table>
		<%} else{ 
			String title="";
		...
		<display:table name="reportList" cellspacing="1" class="table table-bordered" id="lt1">
			...
			<display:column style="text-align:right;" valign="top" title="ESIC Wages"><%=((java.util.List) pageContext.getAttribute("lt1")).get(2)%></display:column>
			<display:column style="text-align:right;" valign="top" title="Employee Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(3)%></display:column>
			<display:column style="text-align:right;" valign="top" title="Employer Contribution"><%=((java.util.List) pageContext.getAttribute("lt1")).get(4)%></display:column>
		</display:table>
		<%} %>
