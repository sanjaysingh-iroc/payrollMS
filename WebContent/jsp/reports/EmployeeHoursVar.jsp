...
function submitForm(type){
	if(type == '1'){
		document.getElementById("strD1").value = "";
		document.getElementById("strD2").value = "";
	}	
	document.frmEmployeeHours.exportType.value='';
	...
	if(type == '2') {
		...
		+'&strLevel='+level+'&paramSelection='+paramSelection;
	}
	//alert("service ===>> " + service);
	$("#divResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
	$.ajax({
		type : 'POST',
		url: 'EmployeeHoursVar.action?f_org='+org+paramValues, 
...
function submitFormSelected(type, cnt){
	if (type == '1') {
		document.getElementById("strD1").value = "";
		document.getElementById("strD2").value = ""+cnt;
	} else if(type == '2') {
		document.getElementById("strD1").value = ""+cnt;
		document.getElementById("strD2").value = "";
	}
	document.frmEmployeeHours.exportType.value='';
	var org = document.getElementById("f_org").value;
	...
	var service = getSelectedValue("f_service");
	var level = getSelectedValue("f_level");
	var strD1 = document.getElementById("strD1").value;
	var strD2 = document.getElementById("strD2").value;
	var paramSelection = getCheckedValue("paramSelection");
	//alert("service ===>> " + service);
	$("#divResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
	$.ajax({
		type : 'POST',
		url: 'EmployeeHoursVar.action?f_org='+org+'&strLocation='+location+'&strDepartment='+department+'&strSbu='+service
			+'&strLevel='+level+'&paramSelection='+paramSelection+'&strD1='+strD1+'&strD2='+strD2, 
		data: $("#"+this.id).serialize(),
		success: function(result){
        	//console.log(result);
        	$("#divResult").html(result);
       ...
       <div class="desgn" style="margin-bottom: 5px;background:#f5f5f5; color:#232323;">
           <div class="box box-primary collapsed-box" style="border-top-color: #EEEEEE;">
               ...
               <div class="box-body" style="padding: 5px; overflow-y: auto; display:none;">
                   <div class="content1">
                       <s:form name="frmEmployeeHours" action="EmployeeHoursVar" theme="simple">
							<s:hidden name="exportType"></s:hidden>
							<s:hidden name="strD1" id="strD1"></s:hidden>
							<s:hidden name="strD2" id="strD2"></s:hidden>
		...
		<%-- <%if(alPaycycleNo.size() > 0 && uF.parseToInt(currentPaycycle) > uF.parseToInt(alPaycycleNo.get(0))){ %>
			<div style="float:left;"><a href="javascript:void(0);" onclick="submitFormSelected('1','<%=alPaycycleNo.get(0)%>');"> << </a></div>
		<%} %>
		<%if(alPaycycleNo.size() > 0 && uF.parseToInt(alPaycycleNo.get(alPaycycleNo.size()-1)) > 1){ %>
			<div style="float:right;"><a href="javascript:void(0);" onclick="submitFormSelected('2','<%=alPaycycleNo.get(alPaycycleNo.size()-1)%>');"> >> </a></div>
		<%} %> --%>
                       ...
                       <%-- <tr>
                           ...
                           <%
                               for (int ii=alPayCycles.size()-1; ii>=0; ii--) {
                               	int count = 1+ii;
							%>
                           <th colspan="3"><%=(String)alPayCycles.get(ii)%></th>
                           <% } %>
                       </tr> --%>	
