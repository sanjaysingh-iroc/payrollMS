		...
		<!-- ====start parvez on 27-10-2022===== -->	
			...
			<div class="widget-user-header bg-aqua-active" style="height: 130px !important;">
				<img class="lazy" src="images1/exusia_banner_<%=alPhotoInner.get(1) %>.png"
					style="display: inline; position: absolute; height: auto !important;"
					data-original='<%=docRetriveLocation +IConstants.I_PEOPLE+"/"+IConstants.I_IMAGE_COVER+"/"+hmEmpProfile.get("EMP_ID")+"/"+hmEmpProfile.get("COVER_IMAGE")%>'>
			<% } else{ %>
			<div class="widget-user-header bg-aqua-active">
				<img class="lazy" src="images1/user-background-photo.jpg"
		...
		<div class="box-body"
			...
			<div id="profilecontainer">
				<div class="content1" style="max-height: 300px; padding: 5px;">
					...
					<% } %>
					<% if(totToDo == 0) { %>
					<div class="tdDashLabel">No to do (approvals).</div>
					<% } %>
				</div>
			</div>
		</div>
				...
				<div class="content1">
					<div class="holder">
						<div style="width: 100%; float: left;">
							...
							<div style="margin: 5px 0px;">
								<a href="Login.action?role=7&userscreen=planLearningGaps"><i
									class="fa fa-caret-right" aria-hidden="true"></i>Plan Learning
									Gaps</a>
							</div>
							<% } %>
							<%if(arrEnabledModules!=null && ArrayUtils.contains(arrEnabledModules, IConstants.MODULE_COMPENSATION_MANAGEMENT+"")>=0) { %>
							<div style="margin: 5px 0px;">
								<a
									href="Login.action?role=7&userscreen=updateStatutoryCompliance"><i
									class="fa fa-caret-right" aria-hidden="true"></i>Update
							...
							<% } %>
							<% } else { %>
							<div class="tdDashLabel">You are not a HR.</div>
							<% } %>
						</div>
					</div>
				</div>
							...
							<div style="float: left; width: 100%; padding: 5px 5px;">
								<div id="eventDataDiv" style="float: left; width: 100%;">
									<div style="float: left; width: 100%;">
										<div style="float: left; width: 94%; margin: 7px;">
											...
											<a href="javascript:void(0)"
												onclick="openEventPopup(<%=event.get(0)%>)"><%=event.get(12)%></a>
											<% } else { %>

											<a href="javascript:void(0)"
												onclick="openEventPopup(<%=event.get(0)%>)">
												<div
													style="float: left; padding: 30px 40px; font-size: 14px; border: 1px solid #CCCCCC;">Image
													Preview not available</div> </a>
											<% } %>
										</div>
									</div>
									<div style="float: left; width: 95%; margin: 3px 7px 3px 7px;">
										<div style="float: left; width: 100%;">
											<div
												style="float: left; width: 100%; font-size: 14px; font-weight: bold; color: #00688B; margin-top: 10px; font-style: italic; margin-left: 7px;">
												<a href="javascript:void(0)"
													onclick="openEventPopup(<%=event.get(0)%>)"><%=event.get(4)%></a>
											</div>
										</div>
										<div style="float: left; width: 100%; margin-bottom: -4px;">
											<div id="blueLikeDiv_<%=event.get(0) %>"
												style="display: block; font-size: 12px; float: left; color: gray; width: 90%; margin-right: 5px">
												<div
													style="float: left; margin-left: 7px; margin-top: -2px;">
													Organised at
													<%=event.get(7) %>
													from
													<%=event.get(1) %>
										...
										<div
											style="float: left; width: 96%; font-style: italic; font-size: 11px; margin-left: 7px; color: gray;">
											Posted on:
											<%=event.get(3) %>.
										</div>
