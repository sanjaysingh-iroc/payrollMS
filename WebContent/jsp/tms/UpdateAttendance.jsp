...
function submitForm(type, strPaycycle) {
	...
	$.ajax({
		type : 'POST',
		url: 'UpdateAttendance.action?btnSubmit=Submit&f_org='+org+paramValues,
...
function approveClockEntries(type){
	if(confirm('Are you sure, you want to update & close time entries of selected employee?')){
		var data = $("#frm_UpdateAttendance").serialize();
		$("#divResult").html('<div id="the_div"><div id="ajaxLoadImage"></div></div>');
		$.ajax({
			type : 'POST',
			url: 'UpdateAttendance.action?formType=approve',
			data: data,
			success: function(result){
	        	$("#divResult").html(result); 
	   		}
...
function calculateAttendance(strEmpId) {
	
	var strTotalDays = document.getElementById("strTotalDays_"+strEmpId).value;
	var strPresentDays = document.getElementById("strPresentDays_"+strEmpId).value;
	var strLeaves = document.getElementById("strLeaves_"+strEmpId).value;
	if(strPresentDays == '') {
		strPresentDays = '0';
	}
	if(strLeaves == '') {
	...
	var strPaidDays = parseFloat(strPresentDays) + parseFloat(strLeaves);
	var strAbsent = parseFloat(strTotalDays) - parseFloat(strPaidDays);
	// Need to remove this if paid extra days 
	if(parseFloat(strPaidDays) > parseFloat(strTotalDays)) {
		alert("Please check, your Paid days are going beyond Total no. of days");
		document.getElementById("strPaidDays_"+strEmpId).value = strTotalDays;
		document.getElementById("strPresentDays_"+strEmpId).value = '0';
		document.getElementById("strAbsent_"+strEmpId).value = '0';
		document.getElementById("strLeaves_"+strEmpId).value = '0';
	} else {
		if(parseFloat(strAbsent)<0) {
			strAbsent = 0;
		}
		document.getElementById("strPaidDays_"+strEmpId).value = strPaidDays;
		document.getElementById("strAbsent_"+strEmpId).value = strAbsent;
	}
}
	...
	%>
		...
		<!-- /.box-header -->
           <div class="box-body" style="width: 100%; padding: 5px; overflow-y: auto; min-height:600px;">
			<s:form theme="simple" name="frm_UpdateAttendance" id="frm_UpdateAttendance" action="UpdateAttendance" method="post">
				...
				<div class="row row_without_margin">
					<div class="col-lg-6 col_no_padding">
						...
						<span id="sendSpan">
							<input type="button" value="Update & Close Time Entries" name="approveSubmit" class="btn btn-primary" onclick="approveClockEntries('2');"/>
						</span>
					</div>
					<div class="col-lg-6">
						<%-- <p style="font-size: 14px;" class="pull-right">
			                <a href="javascript:void(0)" onclick="importAttendance('<%=pageFrom %>');"><i class="fa fa-download" aria-hidden="true"></i>Import Attendance</a>&nbsp;&nbsp;
				...
				<div class="clr margintop20">
					...
					<tbody>
						...
						for (int i = 0; i < nEmpSize; i++) { 
							...
							double nAbsent = 0.0d;
							if(!isAttendance){
								dblPresentDays = uF.parseToDouble(hmPaidDays.get(strEmpId)) - uF.parseToDouble(hmLeavesType.get("COUNT"));
						...
						%> 
							<tr>
								...
								<td class="alignCenter"><%=uF.parseToDouble(strTotalDays)%> 
									<input type="hidden" name="strTotalDays_<%=strEmpId %>" id="strTotalDays_<%=strEmpId %>" value="<%=uF.parseToDouble(strTotalDays) %>"/>
								</td>
								<td class="alignCenter"><input type="text" style="width: 70px !important; text-align: right;" name="strPaidDays_<%=strEmpId %>" id="strPaidDays_<%=strEmpId %>" value="<%=uF.parseToDouble(hmPaidDays.get(strEmpId)) %>" readonly="readonly"/></td>
								<td class="alignCenter"><input type="text" style="width: 70px !important; text-align: right;" name="strPresentDays_<%=strEmpId %>" id="strPresentDays_<%=strEmpId %>" value="<%=dblPresentDays > 0 ? dblPresentDays : uF.parseToDouble(hmPaidDays.get(strEmpId))%>" onkeypress="return isNumberKey(event)" onkeyup="calculateAttendance('<%=strEmpId %>');"/></td>
								<td class="alignCenter"><input type="text" style="width: 70px !important; text-align: right;" name="strLeaves_<%=strEmpId %>" id="strLeaves_<%=strEmpId %>" value="<%=uF.parseToDouble(hmLeavesType.get("COUNT")) %>" onkeypress="return isNumberKey(event)" onkeyup="calculateAttendance('<%=strEmpId %>');"/></td>
								<td class="alignCenter"><input type="text" style="width: 70px !important; text-align: right;" name="strAbsent_<%=strEmpId %>" id="strAbsent_<%=strEmpId %>" value="<%=((nAbsent < 0) ? 0 : nAbsent) %>" readonly="readonly"/></td>
							</tr>
						<%} %>
						<%if (nEmpSize == 0) {%>
							<tr><td colspan="8"><div class="nodata msg" style="width:97%"><span>No employee found.</span></div></td></tr>
						<%}%>
					</tbody>
					</table>
				</div>
