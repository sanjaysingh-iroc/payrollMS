...
//
//public class TimeReportPdf implements ServletRequestAware,
//		ServletResponseAware, IStatements, IConstants {
//
//	UtilityFunctions UF;
//	CommonFunctions CF;
//	HttpSession session;
...
//		System.out.println("printing dates" + getReportdatefrom() + "toooo"
//				+ getReportdateto());
...
//		try {
//			fillsheet(projectdetails, nameheaderlist, leavesdaylist,
//					holidayslist, finaltask, idList, mapconveyancetravel,
//					mapconveyanceothers);
//			createExcelFile(projectdetails, nameheaderlist, leavesdaylist,
//					holidayslist, finaltask, idList, mapconveyancetravel,
//					mapconveyanceothers);
//		} catch (IOException e) {
//			e.printStackTrace();
...
//			List<String> nameheaderlist, List<Integer> leavesdaylist,
//			List<Integer> holidayslist,
//			HashMap<String, ArrayList<String>> finaltask, List<String> idList,
//			Map<String, List<String>> mapconveyancetravel,
//			Map<String, List<String>> mapconveyanceothers) throws IOException {
//		FileOutputStream fos = null;
//		try {
//			writeTimeSheetReport(projectdetails, nameheaderlist, leavesdaylist,
//					holidayslist, finaltask, idList);
//			writeConveyanceSheetReport(nameheaderlist, mapconveyancetravel,
//					mapconveyanceothers);
...
//	public void fillsheet(Map<String, Map<String, String>> projectdetails,
//			List<String> nameheaderlist, List<Integer> leavesdaylist,
//			List<Integer> holidayslist,
//			HashMap<String, ArrayList<String>> finaltask, List<String> idList,
//			Map<String, List<String>> mapconveyancetravel,
//			Map<String, List<String>> mapconveyanceothers) throws SQLException {
//
//		Connection con = null;
//
//		ResultSet rs = null;
...
//		// ******** LEAVES.......
//
//		Map<String, Map<String, String>> hmleaves = CF.getLeaveDates(con,
//				getReportdatefrom(), getReportdateto(), CF, null, true);
...
//			leavesdaylist.add(Integer.parseInt(pairs.getKey().toString()
//					.substring(0, 2)));
//		}
...
//		CF.getHolidayList(getReportdatefrom(), getReportdateto(), CF,
//				hmholidaylist, hmholidays, true);
...
//					.prepareStatement("select client_name,task_date,sum(actual_hrs) as actual_hrs from(select client_name,activity_name,task_date,SUM(actual_hrs) as actual_hrs from(select clnt.client_name,pmnt.client_id ,ai.pro_id,ai.activity_name,ta.activity_id,ta.task_date,ta.actual_hrs from task_activity ta  join activity_info ai on ta.activity_id=ai.task_id  join projectmntnc pmnt on ai.pro_id=pmnt.pro_id join client_details clnt on clnt.client_id=pmnt.client_id where ta.emp_id=?) as a   group by client_name,activity_name,task_date order by client_name)as c where task_date>=? and task_date<=? group by client_name,task_date order by client_name");
//			preparedStatement.setInt(1, UF.parseToInt(getEmpid()));
//			preparedStatement.setDate(2,
//					UF.getDateFormat(getReportdatefrom(), DATE_FORMAT));
//			preparedStatement.setDate(3,
//					UF.getDateFormat(getReportdateto(), DATE_FORMAT));
...
//					Map<String, String> innerpro = projectdetails
//							.get(clientname);
//					innerpro.put(rs.getString("task_date"),
//							rs.getString("actual_hrs"));
...
//
//				mapconveyancetravel.put(
//						no_records + resultSet.getString("client_name"),
//						conveyancelist);
//
//				System.out.println("printing other1111" + mapconveyanceothers);
...
//			statementothers.setDate(1,
//					UF.getDateFormat(getReportdatefrom(), DATE_FORMAT));
//			statementothers.setDate(2,
//					UF.getDateFormat(getReportdateto(), DATE_FORMAT));
...
//				conveyancelist.add(resultSet.getString("reimbursement_amount"));
//
//				if (resultSet.getString("reimbursement_info").equalsIgnoreCase(
//						"Travel")) {
//					mapconveyancetravel
//							.put(no_records
//									+ resultSet.getString("reimbursement_info"),
//									conveyancelist);
//				} else {
//					mapconveyanceothers
//							.put(no_records
...
//				PreparedStatement preptask = con.prepareStatement(strquerytask);
//				preptask.setInt(1, UF.parseToInt(getEmpid()));
//				ResultSet resulttask = preptask.executeQuery();
...
//						List tasklist = finaltask.get(resulttask
//								.getString("client_name"));
//						tasklist.add(resulttask.getString("activity_name"));
//						finaltask.put(resulttask.getString("client_name"),
...
//	}
//
//	public void writeTimeSheetReport(
//			Map<String, Map<String, String>> projectdetails,
//			List<String> nameheaderlist, List<Integer> leavesdaylist,
//			List<Integer> holidayslist,
//			HashMap<String, ArrayList<String>> finaltask, List<String> idList) {
...
//			monthly_time_sheet
//					.addMergedRegion(new CellRangeAddress(0, 0, 0, 10));
//			for (int i = 0; i < 37; i++) {
...
//			monthly_time_sheet
//					.addMergedRegion(new CellRangeAddress(1, 1, 0, 10));
//			for (int i = 0; i < 37; i++) {
...
//			HSSFCellStyle rightBorderForClientSummary = workbook
//					.createCellStyle();
...
//			clientBankACNoCell.setCellValue(" Bank A/C No.: "
//					+ nameheaderlist.get(2));
//
...
//			clientDivisionCell.setCellValue(" Department : "
//					+ nameheaderlist.get(4));
...
//			clientEmpIdCell.setCellValue(" Employee Code : "
//					+ nameheaderlist.get(1));
//			clientEmpIdCell.setCellStyle(clientSummary);
//
//			Cell clientWorkingHrsCell = empCodeRow.createCell(30);
//			clientWorkingHrsCell
//					.setCellValue(" ( Standard Hours per Day  :  8 ) : ");
//
...
//			monthly_time_sheet.addMergedRegion(new CellRangeAddress(12, 12, 1,
//					2));
//
...
//				HSSFCellStyle cellStyleHeadingMonth = workbook
//						.createCellStyle();
...
//								finalmap.put(intdate, mapEntry.getValue()
//										.toString());
//								System.out.println("printing finalmap"
//										+ finalmap);
//
//							}
//							j = j - 1;
...
//			}
//
//			Row totalHrsSpendWithClientsRow = monthly_time_sheet
//					.createRow(linecount);
//			for (int i = 0; i < 37; i++) {
//				HSSFCellStyle cellStyle = workbook.createCellStyle();
//				cellStyle.setBorderTop(CellStyle.BORDER_THIN);
...
//			Row totalHrsSpendWithOtherActivityRow = monthly_time_sheet
//					.createRow(linecount);
//			for (int i = 0; i < 37; i++) {
//				HSSFCellStyle cellStyle = workbook.createCellStyle();
//				cellStyle.setBorderTop(CellStyle.BORDER_THIN);
...
//	}
//
//	public void writeConveyanceSheetReport(List<String> nameheaderlist,
//			Map<String, List<String>> mapconveyancetravel,
//			Map<String, List<String>> mapconveyanceothers) {
...
//			reportNameCell
//					.setCellValue("MONTHLY TIME REPORT (Expense Statement)");
//			reportNameCell.setCellStyle(cellStyleforReportName);
//
...
//			currentDate.setCellValue(" Date : "
//					+ UF.getCurrentDate(CF.getStrTimeZone()));
//			// currentDate.setCellValue(" Date : " + getReportdateto());
//
...
//			statementNameCell
//					.setCellValue(" Conveyance and Other Expenses Statement for the month ending on "
//							+ getReportdateto());
...
//			conveyance_sheet
//					.addMergedRegion(new CellRangeAddress(11, 11, 5, 6));
//
...
//			}
//
//			int k = 0, linecountothers, linecountconveyance, o = 0, linetotal = 0;
...
//							cell.setCellValue(listconven.get(k - 1)
//									.substring(1));
//							innervallist = mapconveyancetravel.get(listconven
//									.get(k - 1));
//						} else if (j > 1 && j < 11) {
//							HSSFCellStyle srNosA = workbook.createCellStyle();
//							srNosA.setAlignment(CellStyle.ALIGN_CENTER);
...
//					if (i >= 25 + linecountconveyance
//							&& i < (25 + linecountconveyance + linecountothers)
//							&& j < 11) {
//						if (j == 0) {
//							o++;
//							HSSFCellStyle srNosA = workbook.createCellStyle();
...
//							cell.setCellValue(listothers.get(o - 1)
//									.substring(1));
//							innervallist = mapconveyanceothers.get(listothers
//									.get(o - 1));
//						} else if (j > 1 && j < 11) {
//							HSSFCellStyle srNosA = workbook.createCellStyle();
//							srNosA.setAlignment(CellStyle.ALIGN_CENTER);
...
//			rule1Cell
//					.setCellValue(" 1) Distance should be calculated from office to Client Place / Pickup Point or Home to Client Place / Pickup Point, whichever is less.");
...
//			rule2Cell
//					.setCellValue(" 2) Rickshaw Charges are not allowed except under exceptional situations and with Partners' Approval only.");
