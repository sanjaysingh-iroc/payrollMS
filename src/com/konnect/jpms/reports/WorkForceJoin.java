...
public class WorkForceJoin extends ActionSupport implements ServletRequestAware, IStatements {
	...
	private static Logger log = Logger.getLogger(WorkForceJoin.class);
	...
	public String execute() throws Exception {
		...
		if(strUserType!=null && !strUserType.equalsIgnoreCase(ADMIN) 
			...
			request.setAttribute(PAGE, PWorkForceJoin);
			viewPayCycle(strP);
		}
...
//			System.out.println("nCount==>"+nCount+" alInnerDates=>"+alInnerDates);
			...
			for (int i = 0; i < alInnerDates.size();) {
				...
				if(strUserType!=null ){ 
					...
					if(strP!=null && strP.equalsIgnoreCase("WLH")){
						pst = con.prepareStatement(selectWorkForceJoinR3_E);
						pst.setDate(1, uF.getDateFormat(strD1, DATE_FORMAT));
						pst.setDate(2, uF.getDateFormat(strD2, DATE_FORMAT));
					}else if(strP!=null && strP.equalsIgnoreCase("DH")){
						pst = con.prepareStatement(selectWorkForceJoinR5_E);
						pst.setDate(1, uF.getDateFormat(strD1, DATE_FORMAT));
						pst.setDate(2, uF.getDateFormat(strD2, DATE_FORMAT));
					}else if(strP!=null && (strP.equalsIgnoreCase("SH"))){
						...
						pst = con.prepareStatement(selectWorkForceJoinR4_E);
						pst.setDate(1, uF.getDateFormat(strD1, DATE_FORMAT));
						pst.setDate(2, uF.getDateFormat(strD2, DATE_FORMAT));
					}else{
						pst = con.prepareStatement(selectWorkForceJoinR3_E);
...
//					hmActualInner.put(x + "L", "<a href=\"ClockEntries.action?T=T&PAY=Y&EMPID=" + strNewId + "&PC=" + x + "&D1=" + strD1 + "&D2=" + strD2 + "\">" + (nActual + 1) + "</a>");
					hmActualInner.put(x + "L", (nActual + 1)+"");
