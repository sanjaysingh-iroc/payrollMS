...
public class SelectCourseAssessmentQuestion extends ActionSupport implements
...
//			System.out.println("Answer Type : "+ansType);
			...
			pst = con.prepareStatement("select * from course_question_bank where answer_type = ?"); // and question_type=? is_add=true and 
			pst.setInt(1, uF.parseToInt(ansType));
			rs = pst.executeQuery();
//			System.out.println("Answer Type pst ::::: "+pst);
			while (rs.next()) {
				sb.append("<option value=\"" + rs.getString("course_question_bank_id")
						+ "\">" + rs.getString("course_question_text").replace("'", "") + "</option>");
			}
			rs.close();
			pst.close();
