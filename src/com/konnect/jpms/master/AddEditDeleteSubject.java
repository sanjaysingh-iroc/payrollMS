...
public class AddEditDeleteSubject extends ActionSupport implements ServletRequestAware, IStatements {
...
//		request.setAttribute(PAGE, PAddService);
		...
		System.out.println("in Subjectdiv add load");
		return LOAD;
	...
	public String loadValidateService() {
		...
		try {
			...
			System.out.println("in addSubjectdiv");
	...
	public String insertSubject() {
		...
		try {
			con = db.makeConnection(con);
			pst = con.prepareStatement("insert into course_subject_details (course_subject_name, subject_desc, org_id, added_by, entry_date) values (?,?,?,?, ?)");
			pst.setString(1, getSubjectName());
			pst.setString(2, getSubjectDescription());
			...
			session.setAttribute(MESSAGE, SUCCESSM+getSubjectName()+" saved successfully."+END);
	...
	public String viewSubject() {
		...
		try {
			...
			pst.setInt(1, uF.parseToInt(getSubjectId()));
			rs = pst.executeQuery();
			while(rs.next()){
				setSubjectId(rs.getString("course_subject_id"));
				setSubjectName(rs.getString("course_subject_name"));
				setSubjectDescription(rs.getString("subject_desc"));
	...
	public String updateSubject() {
		...
		try {
			...
			con = db.makeConnection(con);
			pst = con.prepareStatement("UPDATE course_subject_details SET course_subject_name=?, subject_desc=?, org_id=?, updated_by=?, update_date=? WHERE course_subject_id=?");
			pst.setString(1, getSubjectName());
			pst.setString(2, getSubjectDescription());
			pst.setInt(3, uF.parseToInt(getStrOrg()));
			pst.setInt(4, uF.parseToInt(strSessionEmpId));
			pst.setTimestamp(5, uF.getTimeStamp(uF.getCurrentDate(CF.getStrTimeZone())+""+uF.getCurrentTime(CF.getStrTimeZone()), DBDATE+DBTIME));
			pst.setInt(6, uF.parseToInt(getSubjectId()));
			pst.execute();
			pst.close();

			session.setAttribute(MESSAGE, SUCCESSM+getSubjectName()+" updated successfully."+END);
	...
	public String deleteSubject() {
		...
		try {
			...
			con = db.makeConnection(con);
			pst = con.prepareStatement("delete from course_subject_details where course_subject_id=?");
			pst.setInt(1, uF.parseToInt(getSubjectId()));
			pst.execute();
			pst.close();
			
			session.setAttribute(MESSAGE, SUCCESSM+"Subject deleted successfully."+END);
