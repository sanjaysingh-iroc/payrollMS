...
public class ViewReimbursementDetails extends ActionSupport implements IStatements, ServletRequestAware {
	...
	public String execute() throws Exception {
		...
		viewReimbursementDetails(uF);
		
		return LOAD;		
	}

	private void viewReimbursementDetails(UtilityFunctions uF) {
		Connection con = null;
		...
		try {
			...
			pst.setInt(1, uF.parseToInt(getReimbursementId()));
			rs = pst.executeQuery();
			int nEmpId = 0;
...
//				hmReimbursement.put("REIMB_TO_DATE", uF.getDateFormat(rs.getString("reimb_to_date"), DBDATE, CF.getStrReportDateFormat()));
				...
				if (rs.getString("reimbursement_type1") != null && rs.getString("reimbursement_type1").equalsIgnoreCase("P")) {
					...
					isTravel = true;
					nTravelPlanId = uF.parseToInt(rs.getString("reimbursement_type"));
			...
			if(isTravel && nTravelPlanId > 0){
				pst = con.prepareStatement("select * from emp_leave_entry where emp_id=? and leave_id=? and istravel = true ");
				pst.setInt(1, nEmpId);
				pst.setInt(2, nTravelPlanId);
				rs = pst.executeQuery();
				while(rs.next()){
					hmReimbursement.put("REIMBURSEMENT_TRAVEL_PLAN", uF.showData(rs.getString("plan_name"), ""));
				}
				rs.close();
				pst.close();
			}
			
			request.setAttribute("hmReimbursement", hmReimbursement);
