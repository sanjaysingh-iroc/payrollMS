...
public class TDSExcelReports extends ActionSupport implements ServletRequestAware,ServletResponseAware, IStatements {
...
//		System.out.println("pst==>"+pst);
		...
		while(rs.next()){
			...
			hmEmpInner.put("TDS_AMOUNT", uF.formatIntoTwoDecimalWithOutComma(uF.parseToDouble(rs.getString("amount"))));
...
private void generateTDSReportBy() {
	Connection con = null;
	Database db = new Database();
	db.setRequest(request);
	try {
	...
	while(it.hasNext()){
		...
		String Amount=uF.showData((String)hmInner.get("TDS_AMOUNT"), "0");
	...
	innerList.add(new DataStyle(uF.formatIntoOneDecimalWithOutComma(dblTDSAmountTotal),Element.ALIGN_LEFT,"NEW_ROMAN",10,"0","0",BaseColor.WHITE));
