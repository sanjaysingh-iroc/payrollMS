...
public class ProjectPerformanceReportWP extends ActionSupport implements ServletRequestAware, IStatements{
	/**
	 * 
	 */
	private static final long serialVersionUID = 7282516978313281047L;
	...
	public String execute() {
		...
		request.setAttribute(PAGE, "/jsp/task/ProjectPerformanceReportWP.jsp");
...
//				sbQuery.append(" and pmc.project_owner="+uF.parseToInt(strSessionEmpId));
			...
			/*if(uF.parseToInt(getF_org())>0 || (getF_strWLocation() != null && getF_strWLocation().length>0 && !getF_strWLocation()[0].trim().equals("")) || (getF_department() != null && getF_department().length>0 && !getF_department()[0].trim().equals("")) || (getF_service() != null && getF_service().length>0 && !getF_service()[0].trim().equals("")) || (getF_level() != null && getF_level().length>0 && !getF_level()[0].trim().equals(""))) {
...
//			System.out.println("pst ===>> " + pst);
		...
		//===start parvez date: 18-10-2022===	
			/*sbQuery1.append("select pro_id, pro_name, billing_type, billing_amount, idealtime, start_date, deadline, client_id, actual_calculation_type, " +
				"bill_days_type, hours_for_bill_day, added_by, curr_id, project_owner from projectmntnc pmc where pmc.pro_id > 0 ");
			if(isPoFlag() && uF.parseToInt(getStrProType()) == 2){
				sbQuery1.append(" and pmc.project_owner="+uF.parseToInt(strSessionEmpId));
			}*/
			sbQuery1.append("select pro_id, pro_name, billing_type, billing_amount, idealtime, start_date, deadline, client_id, actual_calculation_type, " +
				"bill_days_type, hours_for_bill_day, added_by, curr_id, project_owners from projectmntnc pmc where pmc.pro_id > 0 ");
			if(isPoFlag() && uF.parseToInt(getStrProType()) == 2){
				sbQuery1.append(" and pmc.project_owners like '%,"+strSessionEmpId+",%' ");
			}
		//===end parvez date: 18-10-2022===	
			if(getProType() == null || getProType().equals("") || getProType().equals("null") || getProType().equals("L")) {
...
//					hmProPerformaceProjectManager.put(rs.getString("pro_id"), uF.showData(hmEmpNMap.get(rs.getString("project_owner")), ""));
					...
					hmProPerformaceProjectManager.put(rs.getString("pro_id"), uF.showData(sbOwners+"", ""));
				}
...
//					String strAmt = uF.formatIntoTwoDecimalWithOutComma(uF.parseToDouble(hmProPerformaceBillable.get(alProjectId.get(i))));
//					sb.append(strAmt);
//					if(i<alProjectId.size()-1) {
//						sb.append(",");	
//					}
//				}
//				sb.append("]},");
...
//				for(int i=0; i<alProjectId.size(); i++) {
//					String strAmt = uF.formatIntoTwoDecimalWithOutComma(uF.parseToDouble(hmProPerformaceBudget.get(alProjectId.get(i))));
//					sb.append(strAmt);
//					if(i<alProjectId.size()-1) {
//						sb.append(",");	
//					}
...
//				for(int i=0; i<alProjectId.size(); i++) {
//					String strAmt = uF.formatIntoTwoDecimalWithOutComma(uF.parseToDouble(hmProPerformaceActual.get(alProjectId.get(i))));
//					sb.append(strAmt);
//					if(i<alProjectId.size()-1) {
//						sb.append(",");	
//					}
...
//				sb.append("{name: '"+hmProName.get(alProjectId.get(i))+"', data:[");
//			
//				sb.append(uF.showData(hmProPerformaceProjectProfit.get(alProjectId.get(i)), "0")+"]},");
//				sbMonth.append("'"+hmProName.get(alProjectId.get(i))+"'"+",");
			}
...
//			System.out.println("pro_amount ===>> " + sb.toString());
