...
public class ProjectWorkAllocation extends ActionSupport implements ServletRequestAware, IStatements {
	...
	public String execute() {
		...
		getEmpWorkAllocationSumary(uF, CF);
			
		return SUCCESS;
	}

	
	public void getEmpWorkAllocationSumary(UtilityFunctions uF, CommonFunctions CF) {
...
//			if(getCurr_date()!=null) {
//				pst = con.prepareStatement("select pmc.actual_calculation_type, ai.activity_name, pmc.pro_name, pmc.client_id, ai.start_date, 
//			ai.deadline, ai.idealtime, ai.already_work, pmc.billing_type, ai.completed from activity_info ai, projectmntnc pmc where pmc.pro_id = ai.pro_id and ai.resource_ids like '%,"+getEmp_id()+",%' and ai.completed <100 and ai.approve_status = 'n' and ? between ai.start_date and ai.deadline order by ai.deadline");
//				pst.setDate(2, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
//			} else {
//				pst = con.prepareStatement("select pmc.actual_calculation_type, ai.activity_name, pmc.pro_name, pmc.client_id, ai.start_date, ai.deadline, ai.idealtime, ai.already_work, pmc.billing_type, ai.completed from activity_info ai, projectmntnc pmc where pmc.pro_id = ai.pro_id and ai.resource_ids like '%,"+getEmp_id()+",%' and ai.completed <100 and ai.approve_status = 'n' order by ai.deadline");
//			}
			...
			if(getCurr_date() != null && !getCurr_date().equalsIgnoreCase("null") && !getCurr_date().equals("")) {
				pst.setDate(1, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
				pst.setDate(2, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
				pst.setDate(3, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
				pst.setDate(4, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
				pst.setDate(5, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
				pst.setDate(6, uF.getDateFormat(getCurr_date(), DATE_FORMAT));
			} else {
