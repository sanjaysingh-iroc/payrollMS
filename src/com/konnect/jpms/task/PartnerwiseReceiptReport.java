...
public class PartnerwiseReceiptReport extends ActionSupport implements ServletRequestAware, ServletResponseAware, IStatements {
	...
	public String execute() throws Exception {
		...
		request.setAttribute(PAGE, "/jsp/task/PartnerwiseReceiptReport.jsp");
		request.setAttribute(TITLE, "Partnerwise Receipt");
	...
	public String viewPartnerwiseReceipt(UtilityFunctions uF) {
		Connection con = null;
		...
		try {
			...
			Map<String, Map<String, String>> hmParnterwiseRecept = new HashMap<String, Map<String,String>>();
...
//			sbQuery.append(" order by project_owner ");
	        ...
	        while(rs.next()) {
				...
				hmInner.put(rs.getString("pro_id")+"_CURR_ID", rs.getString("curr_id"));
			...
			if(strFinancialYearStart != null && !strFinancialYearStart.equalsIgnoreCase("null") && !strFinancialYearStart.equals("") && strFinancialYearEnd != null && !strFinancialYearEnd.equalsIgnoreCase("null") && !strFinancialYearEnd.equals("")) {
			    ...
			    while (it1.hasNext()) {
			    	...
			    	for(int i=0;i<alInner.size() && alInner!=null && !alInner.isEmpty();i++) {
			    		...
			    		Map<String, String> hmCurr = hmCurrencyMap.get(hmProjectData.get(proId+"_CURR_ID"));
						...
						double monthTotPF = 0;
						double monthTotOPE = 0;
						double monthExpTotal = 0;
						...
						while(itr.hasNext()) {
						    ...
						    double profFeesTot = 0;
						    double expecTot = 0;
						    double otherDirCostTot = 0;
							...
							profFeesTot += uF.parseToDouble(hmTotal.get(month+"_PF_M_TOTAL"))+profFees;
							otherDirCostTot += uF.parseToDouble(hmTotal.get(month+"_OPE_M_TOTAL"))+otherDirCost;
							expecTot += uF.parseToDouble(hmTotal.get(month+"_EXPECTED_TOTAL"))+dblBillAmount;
							
							monthTotPF += profFees;
							monthTotOPE += otherDirCost;
							
							hmCommitmentAmt.put(partnerId+"_"+month+"_PF", uF.formatIntoComma(profFees));
							hmCommitmentAmt.put(partnerId+"_"+month+"_OPE", uF.formatIntoComma(otherDirCost));
							
							hmTotal.put(month+"_PF_M_TOTAL", uF.formatIntoComma(profFeesTot));
							hmTotal.put(month+"_OPE_M_TOTAL", uF.formatIntoComma(otherDirCostTot));
						...
						hmCommitmentAmt.put(partnerId+"_TOTAL_PF", uF.formatIntoComma(monthTotPF));
						hmCommitmentAmt.put(partnerId+"_TOTAL_OPE", uF.formatIntoComma(monthTotOPE));
			    	...
			    	totalPF += uF.parseToDouble(hmCommitmentAmt.get(partnerId+"_TOTAL_PF"));
			    	totalOPE += uF.parseToDouble(hmCommitmentAmt.get(partnerId+"_TOTAL_OPE"));
			    	
			    	hmTotal.put("TOTAL", totalCommitment+"");
			    	hmTotal.put("TOTAL_PF", totalPF+"");
			    	hmTotal.put("TOTAL_OPE", totalOPE+"");
			    	
			    	hmParnterwiseRecept.put(partnerId, hmCommitmentAmt);
			    	hmPartner.put(partnerId, hmEmpName.get(partnerId));
			    }
			}
			
//			System.out.println("PWRR/266-monthYearsList="+monthYearsList);
//			request.setAttribute("alPartnerList", alPartnerList);
			...
			request.setAttribute("hmParnterwiseRecept", hmParnterwiseRecept);
	...
	private void generatePartnerwiseReceipt(UtilityFunctions uF) {
...
//			List alPartnerList = (List) request.getAttribute("alPartnerList");
			...
			if(hmPartner == null) hmTotal = new HashMap<String, String>();
			...
			HSSFSheet sheet = workbook.createSheet("Partner wise Receipt Report");
			...
			subHeader.add(new DataStyle("Actually Receipt prof fee till date"+" ("+hmTotal.get("CURRENCY_EXCEL")+")",Element.ALIGN_CENTER,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			subHeader.add(new DataStyle("Expected till date"+" ("+hmTotal.get("CURRENCY_EXCEL")+")",Element.ALIGN_CENTER,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			subHeader.add(new DataStyle("Actual OPE"+" ("+hmTotal.get("CURRENCY_EXCEL")+")",Element.ALIGN_CENTER,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			...
			for(int j=0; j<monthYearsList.size(); j++){
				...
				innerList.add(new DataStyle(uF.showData(hmTotal.get(monthYearsList.get(j)+"_PF_M_TOTAL"),""),Element.ALIGN_LEFT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
				innerList.add(new DataStyle(uF.showData(hmTotal.get(monthYearsList.get(j)+"_EXPECTED_TOTAL"),""),Element.ALIGN_LEFT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
				innerList.add(new DataStyle(uF.showData(hmTotal.get(monthYearsList.get(j)+"_OPE_M_TOTAL"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
				for(int k=0; k<alPartnerId.size(); k++){
					Map<String, String> hmInner = hmParnterwiseRecept.get(alPartnerId.get(k));
					innerList.add(new DataStyle(uF.showData(hmInner.get(alPartnerId.get(k)+"_"+monthYearsList.get(j)+"_PF"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
					innerList.add(new DataStyle(uF.showData(hmInner.get(alPartnerId.get(k)+"_"+monthYearsList.get(j)),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
					innerList.add(new DataStyle(uF.showData(hmInner.get(alPartnerId.get(k)+"_"+monthYearsList.get(j)+"_OPE"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			...
			totInner.add(new DataStyle(uF.showData(hmTotal.get("TOTAL_PF"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			totInner.add(new DataStyle(uF.showData(hmTotal.get("TOTAL"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			totInner.add(new DataStyle(uF.showData(hmTotal.get("TOTAL_OPE"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
			
			
			for(int l=0; l<alPartnerId.size(); l++){
				Map<String, String> hmInner = hmParnterwiseRecept.get(alPartnerId.get(l));
				
				totInner.add(new DataStyle(uF.showData(hmInner.get(alPartnerId.get(l)+"_TOTAL_PF"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
				totInner.add(new DataStyle(uF.showData(hmInner.get(alPartnerId.get(l)+"_TOTAL"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
				totInner.add(new DataStyle(uF.showData(hmInner.get(alPartnerId.get(l)+"_TOTAL_OPE"),""),Element.ALIGN_RIGHT,"NEW_ROMAN",6,"0","0",BaseColor.LIGHT_GRAY));
				
			}
			reportData.add(totInner);
			
			ExcelSheetDesign sheetDesign=new ExcelSheetDesign();
			sheetDesign.generateExcelSheetforPartnerwiseReceipt(workbook,sheet,header,subHeader,rowHeader,reportData);
			
			ByteArrayOutputStream buffer = new ByteArrayOutputStream();
			workbook.write(buffer);
			response.setContentType("application/vnd.ms-excel:UTF-8");
			response.setContentLength(buffer.size());
			response.setHeader("Content-Disposition","attachment; filename=PartnerwiseReceipt.xls");
