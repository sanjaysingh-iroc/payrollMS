...
public class ProjectAgeingReport extends ActionSupport implements ServletRequestAware, IStatements {
	...
	public String execute() throws Exception {
		...
		request.setAttribute(PAGE, "/jsp/task/ProjectAgeingReport.jsp");
		request.setAttribute(TITLE, "Project Ageing Report");
		...
		getProjectAgeingReport(uF);
		
//		System.out.println("poFlag ===>> " + poFlag);
		return loadProjectAgeingReport(uF);

	}
	
	
	public String loadProjectAgeingReport(UtilityFunctions uF) {
...
//			pst.setInt(1, uF.parseToInt(strEmpId));
	...
	private void getProjectAgeingReport(UtilityFunctions uF) {
		Database db = new Database();
		db.setRequest(request);
		Connection con = null;
...
//				int nFYSMonth = uF.parseToInt(uF.getDateFormat(CF.getStrFinancialYearFrom(), DATE_FORMAT, "MM"));
			...
			pst = con.prepareStatement("select sum(received_amount) as received_amount, sum(exchange_rate) as exchange_rate, " +
				"count(exchange_rate) as exchange_rate_cnt, pa.pro_id from promntc_bill_amt_details pa group by pa.pro_id ");
			rs = pst.executeQuery();
			Map<String, String> hmAmountReceived = new HashMap<String, String>();
			while(rs.next()) {
				double exchange_rate_cnt = rs.getDouble("exchange_rate_cnt");
				...
				if(exchange_rate>0) {
					received_amount = rs.getDouble("received_amount")/exchange_rate;
				}
				hmAmountReceived.put(rs.getString("pro_id"), uF.formatIntoOneDecimalWithOutComma(received_amount));
			}
			rs.close();
			pst.close();
			...
			StringBuilder sbQuery = new StringBuilder();			
			sbQuery.append("select sum(invoice_amount) as invoice_amount, p.pro_id from promntc_invoice_details pi, " +
				"projectmntnc p where p.pro_id = pi.pro_id and p.approve_status != 'blocked' ");
...
//				sbQuery.append(" and p.project_owner="+uF.parseToInt(strEmpId));
			...
			//===start parvez date: 14-10-2022===	
				sbQuery.append(" and (p.pro_id in (select pro_id from project_emp_details where _isteamlead = true and emp_id = "
					+uF.parseToInt((String)session.getAttribute(EMPID))+" ) or p.added_by = "+uF.parseToInt((String)session.getAttribute(EMPID))
//					+" or p.project_owner = "+uF.parseToInt((String)session.getAttribute(EMPID))+" ) ");
					+" or p.project_owners like '%,"+(String)session.getAttribute(EMPID)+",%' ) ");
			//===end parvez date: 14-10-2022===	
			}
			...
			sbQuery.append(" group by p.pro_id order by p.pro_id ");
			pst = con.prepareStatement(sbQuery.toString());
//			System.out.println("pst ===>> " + pst);
			rs = pst.executeQuery();
			Map<String, String> hmProInvoiceData = new HashMap<String, String>();
	//		int count=0;
			while(rs.next()) {
				hmProInvoiceData.put(rs.getString("pro_id"), rs.getString("invoice_amount"));
			}
			rs.close();
			pst.close();
			...
			sbQuery.append("select * from projectmntnc where pro_id > 0 and approve_status != 'blocked' ");
...
//			System.out.println("pst ===>> " + pst);
			rs = pst.executeQuery();
			while(rs.next()) {
				if(rs.getString("start_date") == null || rs.getString("start_date").equals("") || rs.getString("deadline") == null || rs.getString("deadline").equals("")) {
					continue;
				}
				Map<String, String> hmProjectData = new HashMap<String, String>();
				List<String> innerList = new ArrayList<String>();
				innerList.add(rs.getString("pro_id")); //0
				innerList.add(rs.getString("pro_name")); //1
				innerList.add(uF.getDateFormat(rs.getString("start_date"), DBDATE, DATE_FORMAT_STR)); //2
				innerList.add(uF.getDateFormat(rs.getString("deadline"), DBDATE, DATE_FORMAT_STR)); //3
				...
				hmProjectData.put("PRO_CAL_TYPE", rs.getString("actual_calculation_type"));
				String strWorkringDays = getProjectActualWorkringDays(con, CF, uF, rs.getString("pro_id"), hmProjectData, true, true);
				String strPlannedDays = getProjectPlannedDays(con, CF, uF, rs.getString("pro_id"), hmProjectData);
				innerList.add(strPlannedDays); //4
				innerList.add(strWorkringDays); //5
				
				if(rs.getString("approve_status") != null && rs.getString("approve_status").equalsIgnoreCase("approved")) {
					if(uF.parseToDouble(strPlannedDays)> uF.parseToDouble(strWorkringDays)) {
						innerList.add("Completed Early"); //6
						double dblDiffDays = uF.parseToDouble(strPlannedDays) - uF.parseToDouble(strWorkringDays);
						innerList.add(uF.formatIntoTwoDecimalWithOutComma(dblDiffDays)); //7
					} else if(uF.parseToDouble(strPlannedDays) < uF.parseToDouble(strWorkringDays)) {
						innerList.add("Overdue"); //6
						double dblDiffDays = uF.parseToDouble(strWorkringDays) - uF.parseToDouble(strPlannedDays);
						innerList.add(uF.formatIntoTwoDecimalWithOutComma(dblDiffDays)); //7
					} else {
						innerList.add("On time"); //6
						innerList.add("0"); //7
					}
				} else {
					innerList.add("Ongoing"); //6
					innerList.add("0"); //7
				}
				String strProInvoiceAmount = uF.showData(hmProInvoiceData.get(rs.getString("pro_id")), "0");
				String strProAmountReceived = uF.showData(hmAmountReceived.get(rs.getString("pro_id")), "0");
				innerList.add(strProInvoiceAmount); //8
				innerList.add(strProAmountReceived); //9
				double dblDueAmount = uF.parseToDouble(strProInvoiceAmount) - uF.parseToDouble(strProAmountReceived);
				innerList.add(uF.formatIntoTwoDecimalWithOutComma(dblDueAmount)); //10
	...
	private String getProjectPlannedDays(Connection con, CommonFunctions CF, UtilityFunctions uF, String proId, Map<String, String> hmProjectData) {
		PreparedStatement pst=null;
		ResultSet rs=null;
		String proPlannedDays = null;
		try {
			StringBuilder sbQuery = new StringBuilder();
			sbQuery.append("select sum(idealtime) as idealtime from activity_info where pro_id = ? ");
...
//			System.out.println("pst======> " + pst);
			rs=pst.executeQuery();
			String proPlannedTime = null;
			while(rs.next()) {
				proPlannedTime = rs.getString("idealtime");
			}
			rs.close();
			pst.close();
			double dblPlannedHrs = uF.parseToDouble(proPlannedTime);
			if(hmProjectData.get("PRO_CAL_TYPE") != null && hmProjectData.get("PRO_CAL_TYPE").equalsIgnoreCase("H")) {
				if(dblPlannedHrs > 0) {
					dblPlannedHrs = dblPlannedHrs / 8;
				}
			} else if(hmProjectData.get("PRO_CAL_TYPE") != null && hmProjectData.get("PRO_CAL_TYPE").equalsIgnoreCase("M")) {
				dblPlannedHrs = dblPlannedHrs * 30;
			}
			proPlannedDays = uF.formatIntoTwoDecimalWithOutComma(dblPlannedHrs);
//		System.out.println(proId + "   hmProActualAndBillableCost ===>> " + hmProActualAndBillableCost);
		...
		return proPlannedDays;
	}


	public String getProjectActualWorkringDays(Connection con, CommonFunctions CF, UtilityFunctions uF, String proId, Map<String, String> hmProjectData, boolean isSubmit, boolean isApprove) {
		PreparedStatement pst=null;
		ResultSet rs=null;
		String proActualWorkringDays = null;
		try {
			StringBuilder sbQuery = new StringBuilder();
			sbQuery.append("select sum(a1.hrs) actual_hrs, sum(a1.days) actual_days from (select sum(ta.actual_hrs) hrs, " +
				"count(distinct ta.task_date) days,activity_id from task_activity ta where task_date between ? and ? ");
			...
			sbQuery.append(" group by activity_id) as a1, activity_info ai where ai.task_id = a1.activity_id and ai.pro_id=? ");
			
			pst = con.prepareStatement(sbQuery.toString()); // and (is_approved = 1 or is_approved = 2) 
			pst.setDate(1, uF.getDateFormat(hmProjectData.get("PRO_START_DATE"), DBDATE));
			pst.setDate(2, uF.getDateFormat(hmProjectData.get("PRO_END_DATE"), DBDATE));
			pst.setInt(3, uF.parseToInt(proId));
//			System.out.println("pst ======>> " + pst);
			rs=pst.executeQuery();
			String proActualTimeHrs = null;
			while(rs.next()) {
				proActualTimeHrs = rs.getString("actual_hrs");
			}
			rs.close();
			pst.close();
			double dblActualHrs = 0;
			if(uF.parseToDouble(proActualTimeHrs) > 0) {
				dblActualHrs = uF.parseToDouble(proActualTimeHrs) / 8;
			}
			proActualWorkringDays = uF.formatIntoTwoDecimalWithOutComma(dblActualHrs);
//		System.out.println(proId + "   hmProActualAndBillableCost ===>> " + hmProActualAndBillableCost);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
