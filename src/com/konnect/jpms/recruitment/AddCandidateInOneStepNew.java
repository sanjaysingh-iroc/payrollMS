...
public class AddCandidateInOneStepNew extends ActionSupport implements ServletRequestAware, IStatements {
	...
	private String strCurrentOrgName;
	...
	private String candiExperienceMonth;
	...
	public String execute() {
		...
		request.setAttribute(PAGE, "/jsp/recruitment/AddCandidateInOneStepNew.jsp");
...
//		System.out.println("getStepSubmit() ===>> " + getStepSubmit());
	...
	private void addCandidateInNewJob(Connection con, UtilityFunctions uF) {
		...
		try {
			...
			if (hmCodeDesig == null)
				hmCodeDesig = new HashMap<String, String>();
...
//				System.out.println("if getJobCode().length ===> " + getJobcode());
				...
				for (int a = 0; a < getJobcode().length; a++) {
					if (uF.parseToInt(getJobcode()[a]) > 0) {
						...
						rs = pst.executeQuery();
//						 System.out.println("ACI1SN/292--pst ===> " + pst);
						while (rs.next()) {
...
//						 System.out.println("checkJobCodeFlag ===> " + checkJobCodeFlag);
						...
						if (!checkJobCodeFlag) {
							...
							if (getAppliSourceType() != null && !getAppliSourceType().equals("")) {
								...
								if (getFromPage() != null && getFromPage().equals("JO") && uF.parseToBoolean(getIsEmpCode())
										&& uF.parseToInt(getRefEmpId()) > 0) {
									pst.setInt(7, SOURCE_REFERENCE);
								} else if (getFromPage() != null && getFromPage().equals("JO") && !uF.parseToBoolean(getIsEmpCode())
										&& uF.parseToInt(getRefEmpId()) == 0) {
							...
							pst.executeUpdate();
//							System.out.println("pst candi123 ===> " + pst);
							pst.close();
...
//				System.out.println("ACI1SN/3758--");
				...
				if ((getApplyType() != null && getApplyType().equalsIgnoreCase("withoutjob"))
						|| (getFromPage() != null && getFromPage().equalsIgnoreCase("CR"))) {
//					System.out.println("if condition");
					sendMail(N_FRESHER_JOB_SUBMISSION, false);
					
		//===start parvez date: 01-11-2021===
				} else if (getFromPage() != null && (getFromPage().equalsIgnoreCase("A")) || getFromPage().equalsIgnoreCase("JO")) {

					sendMail(N_RESUME_SUBMISSION, true);// Created By Dattatray
														// Date:11-08-2021
					sendMailToHiringTeam(N_APPLICATION_SUBMISSION_TO_HIRING_TEAM);    //created by parvez date: 29-10-2021
					
				} 
		//===end parvez date: 01-11-2021===
...
//		System.out.println("notificationCode : " + notificationCode);
		...
		try {
			...
			String job_title = "";
			pst = con.prepareStatement(
					"select cad.candidate_joining_date,job_title,hiring_manager from candidate_application_details cad,recruitment_details rd where rd.recruitment_id =cad.recruitment_id and cad.recruitment_id = ?");
			pst.setInt(1, uF.parseToInt(getRecruitId()));
			rst = pst.executeQuery();
			while (rst.next()) {
...
//			System.out.println("ACIS/448");
			nF.setDomain(strDomain);
			nF.request = request;
			/*System.out.println("ACIS/450");
...
//			System.out.println("ACI1SN/465--getStrEmailTo"+nF.getStrEmailTo());
...
//		System.out.println("AC1S/522--notificationCode : "+notificationCode);
		...
		try {
				...
				nF.setStrCandiLname(getEmpLname());
//					nF.setStrJobTitle(job_title);
				nF.setStrJobTitle(getStrJobTitle());
				...
				nF.setStrCandiCurrentLocation(getCurrentLocation());
//					nF.setStrCandiPreferredLocation(uF.getAppendData(getStrWLocation()));
				nF.setStrCandiPreferredLocation(CF.getCandidatePreferedLocation(con,uF.getAppendData(getStrWLocation())));
				nF.setStrCandiCurrentOrg(getStrCurrentOrgName());
				nF.setStrCandiMobileNo(getEmpMobileNo());
				nF.setStrCandiEmail(getEmpEmail());
				nF.setStrCandiPrimarySkill(uF.getAppendData(skillName));
				nF.setStrCandiSecondarySkill(uF.getAppendData(skillNameSec));
...
//					System.out.println("ACI1S/553--getPrefLocation="+getPrefLocation());
//					System.out.println("ACI1S/553--getEmpMobileNo="+getEmpMobileNo());
//					System.out.println("ACI1S/554--getEmpEmail="+getEmpEmail());
//					System.out.println("ACI1S/555--skillName="+uF.getAppendData(skillName));
//					System.out.println("ACI1S/558--uF.getAppendData(skillNameSec)="+uF.getAppendData(skillNameSec));
//					System.out.println("ACI1S/564--nF.getStrCandiEmail="+nF.getStrCandiEmail());
//					System.out.println("ACI1S/565--nF.getStrCandiMobileNo="+nF.getStrCandiMobileNo());
	...
	public String loadValidateEmployee() {
		...
		try {
			...
			sbSkills.append(" <table><tr><td>" + "<select name=skillName style=margin-bottom:7px; class=form-control auto>"
					+ "<option value=>Select Skill Name</option>");
			for (int k = 0; k < skillsList.size(); k++) {
				sbSkills.append("<option value=" + ((FillSkills) skillsList.get(k)).getSkillsId() + "> " + ((FillSkills) skillsList.get(k)).getSkillsName()
						+ "</option>");
			}
			sbSkills.append(
					"</select></td><td>" + "<select name=skillValue style=margin-left:10px;margin-bottom:7px;width:100px!important; class=form-control auto>"
	...
	private void uploadImage(int empId2) {
		...
		try {
			...
			uI.setEmpId(empId2 + "");
			uI.setCF(CF);
			// System.out.println("empId2 ===> "+empId2+" getEmpImage() ===>
			// "+getEmpImage());
			uI.upoadImage();
		...
		try {
			...
			if (idDoc != null) {
				if (x > 0) {
					...
					String strProcessMsg = uF.showData(strProcessByName, "") + " has inserted document (" + uF.showData(idDocName, "") + ") of "
							+ uF.showData(strCandiName, "") + " on " + ""
...
//			System.out.println("idDoc ===>> " + idDoc);
			if (idDoc != null) {
				...
				pst = con.prepareStatement(
						"INSERT INTO candidate_documents_details(documents_name, documents_type, emp_id, documents_file_name,added_by,entry_date) values (?,?,?,?,?,?)");
				pst.setString(1, idDocName);
...
//				System.out.println("ACI1S/799--getSkillName()[i]="+getSkillName()[i]);
	...
	private String addSkillsDetails(Connection con, PreparedStatement pst, ResultSet rs, String skillName, UtilityFunctions uF) throws SQLException {
		String skillId = null;
	...
	public int insertCandidatePersonalDetails(Connection con, UtilityFunctions uF, CommonFunctions CF) {
		...
		try {
			...
			pst.setDouble(17, uF.parseToDouble(getCandiTotalExperience() + "." + getCandiExperienceMonth()));
			pst.setString(18, uF.getAppendData(getStrWLocation()));
//			System.out.println("pst candi1 ===> " + pst);
			pst.execute();
			pst.close();
...
//			System.out.println("pst candi max id ===> " + pst);
			...
			pst = con.prepareStatement(
					"INSERT INTO candidate_prev_employment (company_name,company_location,designation,emp_id,from_date" + ") VALUES (?,?,?,?, ?)");
			pst.setString(1, getStrCurrentOrgName());
			pst.setString(2, getCurrentLocation());
			pst.setString(3, getStrJobTitle());
	...
	public String getStrCurrentOrgName() {
		return strCurrentOrgName;
	}

	public void setStrCurrentOrgName(String strCurrentOrgName) {
		this.strCurrentOrgName = strCurrentOrgName;
	}
	...
	public String getCandiExperienceMonth() {
		return candiExperienceMonth;
	}

	public void setCandiExperienceMonth(String candiExperienceMonth) {
		this.candiExperienceMonth = candiExperienceMonth;
	}
